...
- 學生任務：
  - 完成一個不計分的操作練習；
  - 完成五個由簡至繁的駕駛關卡；
  - 在每段指定道路內控制車輛符合指定運動規律；
  - 使用角落預覽視窗切換 `x–t`、`v–t` 或隱藏圖像；
  - 完成一個佔少量分數的圖像證據 checkpoint；
  - 檢查已記錄的關卡表現並一次過提交。
- 主要互動：
  - 直接按住油門或煞車踏板的輕／中／盡區域，放手解除；
  - 保持按住時可在同一踏板內左右滑動改變力度；
  - 暫停、繼續及重開當前試車；
  - 切換 `x–t`／`v–t`／隱藏圖像；
  - 在回放分析中拖動只讀時間游標；
  - 接受當前表現或重新挑戰；
...
油門及煞車各是一個連續的大型踏板表面；每個表面由左至右標示 `輕`／`中`／`盡`。學生直接按住所需區域，不需要先選力度再按另一個掣：

- `pointerdown`：依按下位置直接施加輕、中或盡力度；
- 保持按住：持續施加該力度；
- active hold 期間左右移動：在同一踏板內即時切換力度；
...
- `Q`／`W`／`E`：分別按住輕／中／盡油門；`ArrowUp` 等同中油門；
- `A`／`S`／`D`：分別按住輕／中／盡煞車；`ArrowDown` 等同中煞車；
- `Space`：暫停／繼續；
- 所有功能亦有可聚焦按鈕；
...
- 縱軸向上及橫軸向右必須有清楚箭嘴，明示兩個物理量的正方向；箭嘴、軸符號及圖線在最窄支援 viewport 亦不可被卡片邊界裁走。
- 控制面板、關卡指示、分析回饋、checkpoint 及提交後檢討中的物理量符號使用同一數學排版元件：變量 `x`、`v`、`t` 為數學斜體，運算符保持正體；短公式不為此額外載入 MathJax。
...
- 即時預覽由同一次試車起點開始累積；進入新路段不得清空、重設或換成另一條新線。
- `x–t` 圖顯示由整次試車起點量度的路線位置；`v–t` 圖顯示同一次試車的向前速度。
- 第一個水平時間跨度為 `12 s`，縱軸固定為 `20` 個內部速度單位；只完成部分路線時，圖線只畫到實際時間位置，不拉伸至整個圖窗。
- 若試車超過目前時間跨度，時間軸以 `12 → 24 → 48 → 96 s` 的倍增方式擴展並重畫完整歷史；不得刪除較早樣本或使用只保留最近資料的 sliding window。
- 擴展只改變顯示比例，不改變權威樣本、評分或圖線的直／彎分類。回放及 restore 必須按相同規則得到相同範圍。
- 分區分析畫面可以單獨顯示所選計分區，並以該區入口作分析時間原點；這個分區檢視不適用於駕駛中的即時預覽。
...
- 坡度轉折可與清楚標示的路段分界線重合；分界前後各自是固定坡度直路；
...
- 無指定目標速度；
- 空檔時道路阻力令車減速，不能取得滿分；
- 學生要比較三種油門力度，試出能剛好抵消阻力的固定力度；
- 權威滿分策略是由路段起點持續按住輕油門，但 learner-facing 提示不直接揭示；
...
- 輕油門令加速度逐漸減弱，`v–t` 圖向下彎；
- 中油門在此速度範圍提供固定淨加速度，是唯一滿分固定策略；
- 油門踩盡令加速度隨速度增加，`v–t` 圖向上彎；
- 學生不需要在路段中途高頻切換，而是透過重試比較三條圖線，找出唯一筆直的一條。
...
- 道路為平地；
- 不指定減速度大小；
- 空檔及輕煞車都會減速，但減速度隨速度改變，`v–t` 圖不是直線；
- 中煞車提供固定淨減速度，是唯一滿分固定策略；
- 煞車踩盡的減速度變化更大，並可能過早停車；
- 學生透過重試比較圖線，不需要在同一次 run 內高頻切換；
- 車必須保持向前並完成整個區域；
- 在區域中過早停車不算成功的勻減速；
- 突然重煞後長時間慢速滑行不算勻減速。
...
> 在一條完整上斜坡試出合適的油門力度，保持勻速。

設計：

- 全關由起點至終點只有一段固定坡度的上斜路，途中沒有平路、坡度轉折或落斜；
- 整段為一個 15 分計分區；
- 車以基準速度進入，上斜會令不合適的控制顯示為向上或向下的 `v–t` 圖；
- 權威滿分策略是持續按住中油門，但 learner-facing 提示不直接揭示；
- 選對力度後保持按住至終點，不需要途中切換或微調。

重點：

- 比較平路與上斜保持勻速時所需的油門力度；
- 勻速仍以水平 `v–t` 圖判斷；
- 難度來自試出合適力度，不來自記住或追趕坡度轉折。
...
- 路線長 `267` 個內部距離單位：平路勻速 `0–70`、平路勻加速 `70–150`、平路勻減速 `150–187`、落斜勻速 `187–267`；
- 第一個路牌及其後每次運動要求改變時，都在路牌正下方畫一條橫跨馬路的黃色分界線；不可出現只有路牌而沒有黃線的情況；
- 權威位置是車頭；黃線接觸車頭的一刻才切換要求及評分路段，車頭之前的車身長度不可令切換提早；
- 第一條黃線是正式起步線，車頭初始位置就在該線上；開始試車後立即按第一個路牌要求操作；
- 車越過分界線後立即按該路牌要求操作；畫面不為黃線前的道路另加名稱，也不顯示「下一區」說明；
- 舞台左上角只重複目前路段要求，例如「保持勻加速」；
- 四段道路在系統內各佔 5 分，但學生只需理解路牌與分界線；
...
這不是完整汽車工程模型，而是為本活動的「操作—圖像—判斷」證據鏈而設：

- 平路空檔：只有阻力，車逐漸減速；
- 平路、初速 `8 m/s`、輕油門：驅動剛好抵消阻力，形成勻速；
- 平路、中油門：淨加速度固定為正值，形成勻加速；
- 平路、油門踩盡：淨加速度隨速度增加，`v–t` 圖向上彎；
- 平路、輕油門：車速接近平衡速度時加速度逐漸減弱，`v–t` 圖向下彎；
- 平路、中煞車：淨加速度固定為負值，形成勻減速；
- 空檔、輕煞車及煞車踩盡：減速度隨速度改變，不能冒充勻減速。

學生不需要在同一平路關卡內高頻切換；正確學習操作是分別試車、觀察無數字 `v–t` 圖，再保留唯一形成直線的固定策略。
...
- 相鄰路段共用同一個連續運動狀態；學生在分界線位置切換控制。
- 分界 crossing 與 Canvas 使用相同的車頭參考點；不能以車身中央計分但以車頭呈現過線。
...
S_linear = fullThenFade(normalizedRMSE, 0.0015, 0.008)
...
`S_linear` 的容差特意要求預覽中接近真正直線：錯誤固定力度雖然方向正確，仍會因可見曲率明顯失去線性分，避免「任何油門都幾乎滿分」。
...
- 車頭固定朝右，前燈、前保險桿及鬼面罩在右側，尾燈在左側，避免車頭方向含糊；
- 車頭作權威位置參考；道路分界線接觸可見前保險桿時，物理模型及評分同時越界；
- 車身按道路局部斜度旋轉；
- 車輪按沿道路距離順時針轉動，與向右前進的 Canvas 畫面方向一致；
- 停止及暫停時輪胎停止；
- 不用速度線作必要證據。
...
- 道路以有明確遠近邊界的單一行車線呈現，不畫中央分隔虛線；遠側設行人路、草坡、建築及樹木，近側設路肩、行人路與前景草地，避免畫面成為整片灰色；
- 道路坡度及目標區邊界比裝飾更清楚；
...
- 油門及煞車各是一個大型按住表面，內含三個足夠寬的力度區；學生在一次 pointerdown 直接選擇力度，不設獨立力度 selector。
- active hold 可在同一踏板內左右移動，pressed state 及文字狀態同步顯示目前力度。
- 目前路段目標以文字、圖案及顏色共同表達。
...
- 聲音不是必要資訊；第一版預設無聲。
- 焦點順序跟隨任務、開始／暫停、踏板、圖像及 navigation。
...
- 空檔、三段油門、三段煞車的 acceleration ordering；
- 阻力隨速度平方增加；
- 輕油門趨向基準速度、中油門保持固定加速度、油門踩盡的加速度隨速度增加；
- 中煞車保持固定減速度，其他減速策略留下曲線；
...
- 第 5 關第一條黃線固定在起點 `0`，車頭初始位置與它重合；其後三條 boundary marker 固定在 `70, 150, 187`；
- 車頭接觸每條黃線時，物理 segment、評分 segment 及可見 crossing 同步；按車頭切換的滿分策略必須得到 `20/20`；
...
- 介面沒有「先選力度再按踏板」流程；每個踏板的輕／中／盡區可直接按住。
- active hold 期間可在同一踏板左右滑動改變力度，正常操作不產生 pointercancel。
- 油門與煞車不能同時疊加。
...
- 第 5 關每個運動要求路牌（包括第一個）都在同一位置畫一條跨越整個馬路闊度的分界線。
- 車頭是可見及權威 crossing 參考；黃線接觸前保險桿時才切換路段，不可要求學生等黃線到車身中央。
- 預覽圖沒有任何數字刻度或數值。
