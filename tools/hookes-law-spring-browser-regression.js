...
async function completeLearnerPath(cdp, baseUrl, launchPath, label, keyboard = false) {
  ...
  const designInitial = await evaluate(cdp, "(() => { const zero=[...document.querySelectorAll('#stageSvg text')].find((node)=>node.textContent==='0.0 cm'),ceiling=[...document.querySelectorAll('#stageSvg line')].find((node)=>node.getAttribute('x1')==='96'&&node.getAttribute('x2')==='625'&&node.getAttribute('y1')==='62'),ceilingLabel=[...document.querySelectorAll('#stageSvg text')].find((node)=>node.textContent==='固定端／天花板'),limitLine=[...document.querySelectorAll('#stageSvg line')].find((node)=>node.getAttribute('x1')==='142'&&node.getAttribute('x2')==='705'&&node.getAttribute('stroke')==='#dc2626'),emptyHeading=[...document.querySelectorAll('#stageSvg text')].find((node)=>node.textContent==='先在左側選擇彈簧及負載。'),emptyHelp=[...document.querySelectorAll('#stageSvg text')].find((node)=>node.textContent.includes('圖台會用你的斜率')); return {heading:document.querySelector('#designPanel h2')?.textContent||'',calculationState:document.getElementById('designCalculation')?.dataset.state||'',stageText:[...document.querySelectorAll('#stageSvg text')].map((node)=>node.textContent),zeroY:Number(zero?.getAttribute('y')),ceilingY:Number(ceiling?.getAttribute('y1')),ceilingEndX:Number(ceiling?.getAttribute('x2')),ceilingLabelX:Number(ceilingLabel?.getAttribute('x')),limitY:Number(limitLine?.getAttribute('y1')),emptyHeadingY:Number(emptyHeading?.getAttribute('y')),emptyHelpY:Number(emptyHelp?.getAttribute('y'))}; })()");
