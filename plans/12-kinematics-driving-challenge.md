...
- 車越過分界線後立即按該路牌要求操作；畫面不設準備區、轉換區、計分區等額外名目，也不顯示「下一區」說明；
...
`physicsVersion = 5` 包含速度相依阻力、三段踏板反應及 boundary replay 規則；`levelSetVersion = 8` 包含自由跳關、單一完整上斜坡第 4 關，以及只用「路牌＋同位置分界線」的四段混合路線。早期 pre-release 的 version 1–7 snapshot 不作隱式遷移，decoder 必須 fail closed，測試亦要覆蓋舊 physics 及舊 level-set rejection。
...
- 第 5 關第一個要求路牌亦有同位置黃線，其後三條 boundary marker 固定在 `70, 150, 187`，每一條與新要求路牌使用相同世界位置；
