...
async function semanticSmoke(cdp, url, label) {
  ...
  const drawnPrediction = await evaluate(cdp, `(() => { const state=window.__staticKineticFrictionApp.getState(),label=document.querySelector('#apparatusSvg text.force-builder-label'),output=document.querySelector('[data-prediction-magnitude-readout]'); return { direction:state.predictions[0]?.direction, magnitudeCN:state.predictions[0]?.magnitudeCN, arrow:Boolean(document.querySelector('.prediction-friction-arrow')), readout:output?.textContent, forceLabelFont:label ? parseFloat(getComputedStyle(label).fontSize) : 0, readoutFont:output ? parseFloat(getComputedStyle(output).fontSize) : 0 }; })()`);
