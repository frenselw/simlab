(function (root, factory) {
  ...
  function semantic(state, problem, legacyStrict = false) {
    ...
    const lineLegal = legacyStrict ? Model.lineValid : Model.lineRecordable;
    if (new Set(lineKeys).size !== lineKeys.length || !state.part2.lines.every((line) => exact(line, ["holeKey", "a", "b"]) && hangKeys.includes(line.holeKey) && lineLegal(line, holeMap.get(line.holeKey), problem.part2.centre, problem.part2.size))) return null;
    if (legacyStrict && state.part2.activeHoleKey !== null && lineKeys.includes(state.part2.activeHoleKey) || hangKeys.some((key) => key !== state.part2.activeHoleKey && !lineKeys.includes(key))) return null;
    ...
    if (state.part2.mark && (hangKeys.length < 2 || state.part2.lines.length < 2 || !enoughAngle || legacyStrict && state.part2.activeHoleKey !== null)) return null;
  ...
  function validate(state, review = false) {
    if (!exact(state, ["v", "generatorVersion", "rubricVersion", "seed", "phase", "variant", "part1", "part2", "part3"]) || state.v !== VERSION ||
      !Generator.SUPPORTED_VERSIONS.includes(state.generatorVersion) || state.rubricVersion !== Scoring.RUBRIC_VERSION || !Number.isInteger(state.seed) || state.seed < 0 || state.seed > 0xffffffff ||
  ...
  function migrateV1(value) {
    ...
    const facts = semantic(value, problem, true); if (!facts) return null;
  ...
  function settleHole(state, key) { const problem = state && Generator.generate(state.seed, state.generatorVersion); if (!problem?.part2.holes.some((x) => x.key === key) || state.part2.activeHoleKey) return null; return mutate(state, "part2", (next) => { if (!next.part2.hangRecords.some((x) => x.holeKey === key)) next.part2.hangRecords.push({ holeKey: key }); next.part2.activeHoleKey = key; }); }
  function detachActiveHole(state) { if (!state?.part2?.activeHoleKey) return null; return mutate(state, "part2", (next) => { const key = next.part2.activeHoleKey, hasLine = next.part2.lines.some((line) => line.holeKey === key); next.part2.activeHoleKey = null; if (!hasLine) next.part2.hangRecords = next.part2.hangRecords.filter((record) => record.holeKey !== key); }); }
  function traceVertical(state) { if (!state?.part2?.activeHoleKey) return null; const problem = Generator.generate(state.seed, state.generatorVersion), hole = problem.part2.holes.find((x) => x.key === state.part2.activeHoleKey), dx = problem.part2.centre.x - hole.x, dy = problem.part2.centre.y - hole.y, n = Math.hypot(dx, dy), length = .55 * problem.part2.size; return recordLine(state, { holeKey: hole.key, a: [hole.x, hole.y], b: [hole.x + dx / n * length, hole.y + dy / n * length] }); }
  function recordLine(state, line) { if (!state?.part2?.activeHoleKey || line?.holeKey !== state.part2.activeHoleKey) return null; const problem=Generator.generate(state.seed, state.generatorVersion),hole=problem.part2.holes.find((item)=>item.key===line.holeKey);if(!Model.lineRecordable(line,hole,problem.part2.centre,problem.part2.size))return null;return mutate(state, "part2", (next) => { next.part2.lines = next.part2.lines.filter((x) => x.holeKey !== line.holeKey); next.part2.lines.push(clone(line)); }); }
  function markPart2(state, mark) { if (!validPoint(mark)) return null; return mutate(state, "part2", (next) => { next.part2.mark = clone(mark); }); }
  function clearMarkPart2(state) { return mutate(state, "part2", (next) => { next.part2.mark = null; }); }
  function setView(state, view, record = true) { const current = Model.canonicalView(view); if (!current) return null; return mutate(state, "part3", (next) => { next.part3.view = current; const problem = Generator.generate(next.seed, next.generatorVersion); if (record && next.part3.observations.length === 0 && Model.orientationDifference(problem.part3.initialView, current) >= 21) next.part3.observations.push(current); else if (record && next.part3.observations.length === 1 && Model.orientationDifference(next.part3.observations[0], current) >= 36) next.part3.observations.push(current); }); }
  ...
  return { VERSION, initial, variantFor, validate, encode, decode, migrateV1, release, markPart1, settleHole, detachActiveHole, traceVertical, recordLine, markPart2, clearMarkPart2, setView, selectPart3, allComplete, switchPart, enterCheck, resetPart, makeReview, fromReview, lifecyclePolicy };
});
