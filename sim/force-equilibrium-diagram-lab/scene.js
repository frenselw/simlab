(function (root, factory) {
  ...
  function arrowPath(start, end) {
    ...
    const head = Math.min(14, length * .4), half = Math.min(6, length * .18), shaft = 1.7;
  ...
  function sceneMarkup(q, l) {
    const { width: w, height: h, center: c } = l;
    const slope = ["A", "D"].includes(q.family) ? q.params.sign * q.params.theta : 0;
    const blockWidth = Math.min(100, Math.max(66, w * .14)), blockHeight = blockWidth * .59;
    ...
    if (q.motion) {
      html += `<g data-background="far" fill="none" stroke="#dbe5ea" stroke-width="2">`;
      for (let x = -216; x < w + 216; x += 216) html += `<path d="M${x},${c.y - 30}v-33h25v33m-12,-33v-10m-22,43h46"/>`;
      html += "</g>";
    }
    if (q.surface !== "none") {
      ...
      html += '<g data-background="ground" stroke="#c7c3b8" stroke-width="1.4">';
      for (let x = -72; x < w + 72; x += q.surface === "rough" ? 18 : 72)
        html += line({ x, y: y(x) + 4 }, { x: x - 6, y: y(x) + (q.surface === "rough" ? 12 : 5) });
      html += "</g>";
    }
    const reach = angle => {
    ...
    q.ropes.forEach(a => {
      ...
      html += line(start, end, 'class="scene-rope" stroke="#8a7759" stroke-width="3"');
    ...
    q.rods.forEach((f, i) => {
      ...
      html += line(start, end, 'class="scene-rod" stroke="#94a3b8" stroke-width="6" stroke-linecap="round"');
      const label = point(c, a, reach(a));
      html += `<text class="rod-label" x="${label.x}" y="${Math.max(58, label.y - 7)}" text-anchor="middle" font-size="14" font-family="system-ui" fill="#475569">${"ABC"[i]}</text>`;
    });
    ...
    html += `<circle class="center-mark" cx="${c.x}" cy="${c.y}" r="3.5" fill="#334155"/><text x="${c.x + 7}" y="${c.y - 6}" font-size="12" font-family="system-ui" fill="#475569">重心</text>`;
    return { html, body: { x: c.x - blockWidth / 2 - 6, y: c.y - blockHeight / 2 - 6, width: blockWidth + 12, height: blockHeight + 12 } };
  }
  ...
  function labelPositions(records, l, body) {
    ...
    return entries.map(entry => {
      const u = unit(entry.record[1] / 10), n = { x: -u.y, y: u.x }, width = entry.label.sub ? 27 : 19, height = 21;
      const options = [];
      for (const side of [1, -1]) for (const offset of [15, 28, 42]) for (const along of [9, -12]) {
        const x = M.clamp(entry.end.x + n.x * offset * side + u.x * along - width / 2, 6, l.width - width - 6);
        const y = M.clamp(entry.end.y + n.y * offset * side + u.y * along - height / 2, 43, l.height - height - 20);
  ...
  function render(svg, q, l, records, selected = -1, guideAngle = null) {
    ...
    let html = base.html;
    if (guideAngle !== null) html += line(point(l.center, guideAngle, -55), point(l.center, guideAngle, 95), 'stroke="#94a3b8" stroke-width="1" stroke-dasharray="3 4"');
    const positions = labelPositions(records, l, base.body);
    html += '<g class="student-arrows">';
    for (const { i, end, label } of positions) html += `<path data-arrow="${i}" d="${arrowPath(l.center, end)}" fill="${label.color}" opacity="${selected === -1 || selected === i ? 1 : .82}"/><circle cx="${end.x}" cy="${end.y}" r="2.5" fill="#fff" stroke="${label.color}"/>`;
    html += "</g><g class=\"force-labels\">";
    for (const { box, label } of positions) html += `<text class="force-label" x="${box.x + box.width / 2}" y="${box.y + 16}" text-anchor="middle" font-size="18" fill="${label.color}" stroke="#fff" stroke-width="4" paint-order="stroke" stroke-linejoin="round">${label.symbol}${label.sub ? `<tspan font-size="12" dy="4" font-style="normal">${label.sub}</tspan>` : ""}</text>`;
    svg.innerHTML = html + "</g>";
  }
  function animate(svg, elapsed, motion) {
    const ground = svg.querySelector('[data-background="ground"]'), far = svg.querySelector('[data-background="far"]');
    if (ground) ground.setAttribute("transform", `translate(${M.backgroundOffset(elapsed, motion)} 0)`);
    if (far) far.setAttribute("transform", `translate(${M.backgroundOffset(elapsed * .45, motion, 216)} 0)`);
  }
