(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function render() {
      ...
      const complete=[state.part1.markX!==null,Boolean(state.part2.mark),state.part3.selectedCandidateKey!==null],count=complete.filter(Boolean).length;dom.progress.value=count;dom.badge.textContent=state.phase==="check"?"提交前檢查":`第 ${Number(state.phase.slice(-1))} 部分`;
    ...
    function renderPart1(visualsOnly = false) {
      ...
      const defs=svgEl("defs"),rod=svgEl("linearGradient",{id:"rod",x1:"0",y1:"0",x2:"0",y2:"1"});rod.append(svgEl("stop",{offset:"0","stop-color":"#f8fafc"}),svgEl("stop",{offset:".45","stop-color":"#cbd5e1"}),svgEl("stop",{offset:"1","stop-color":"#94a3b8"}));defs.append(rod);
      dom.svg.replaceChildren(defs,svgEl("rect",{x:0,y:0,width:700,height:460,fill:"#ffffff",class:"blank-stage"}),svgEl("line",{x1:42,y1:370,x2:658,y2:370,stroke:"#e5e7eb","stroke-width":1}));
      const apparatus=svgEl("g",{transform:`rotate(${angle} ${x} ${rodBottom})`});
      apparatus.append(svgEl("ellipse",{cx:350,cy:258,rx:265,ry:8,fill:"#0f172a",opacity:".08"}),svgEl("rect",{x:80,y:215,width:540,height:24,rx:12,fill:"url(#rod)","data-scene":"rod","data-contact-y":rodBottom}));
      ...
      dom.svg.append(svgEl("path",{d:`M${x-18} 338 L${x-4} ${rodBottom+1} Q${x} ${rodBottom-1} ${x+4} ${rodBottom+1} L${x+18} 338Z`,fill:"#64748b","data-scene":"support","data-contact-y":rodBottom}),svgEl("rect",{x:x-28,y:338,width:56,height:7,rx:3.5,fill:"#475569"}));
      if (visualsOnly) return;
      const support = directButton(x / 7, rodBottom/4.6, "承托點；左右方向鍵移動，Enter 放手測試", "support"); support.classList.add("support-target"); support.addEventListener("keydown", (event) => { if (event.key === "ArrowLeft" || event.key === "ArrowRight") { supportX = Model.clamp(supportX + (event.key === "ArrowLeft" ? -1 : 1) * (event.shiftKey ? .05 : .01), 0, 1); event.preventDefault(); render(); } else if (event.key === "Enter" || event.key === " ") { event.preventDefault(); testSupport(); } }); support.addEventListener("pointerdown", (event) => beginDrag(event, support, "support", (point) => { supportX = Model.clamp((point.x - 80) / 540, 0, 1); support.style.left = `${(80 + supportX * 540) / 7}%`; renderPart1(true); }, testSupport));
    ...
    function renderPart2(visualsOnly = false) {
      const defs=svgEl("defs"),plateFill=svgEl("linearGradient",{id:"plateFill",x1:"0",y1:"0",x2:"1",y2:"1"});plateFill.append(svgEl("stop",{offset:"0","stop-color":"#93c5fd","stop-opacity":".58"}),svgEl("stop",{offset:"1","stop-color":"#3b82f6","stop-opacity":".48"}));defs.append(plateFill);
      dom.svg.replaceChildren(defs,svgEl("rect", { x:0,y:0,width:700,height:460,fill:"#ffffff",class:"blank-stage" }),svgEl("line",{x1:24,y1:86,x2:676,y2:86,stroke:"#e5e7eb","stroke-width":1}),svgEl("circle",{cx:350,cy:42,r:31,fill:"#dbeafe",opacity:.72}),svgEl("circle",{cx:350,cy:42,r:5,fill:"#1e40af"}),svgEl("path",{d:platePath(),class:"plate",fill:"url(#plateFill)"}));
      ...
      if(drawGhost)dom.svg.append(svgEl("line",{x1:drawGhost.a.x,y1:drawGhost.a.y,x2:drawGhost.b.x,y2:drawGhost.b.y,stroke:drawGhost.snapped?"#2563eb":"#94a3b8","stroke-width":2,"stroke-dasharray":"5 4"}));
      ...
      for (const hole of problem.part2.holes) { const p = localToSvg(hole); dom.svg.append(svgEl("circle", { cx:p.x,cy:p.y,r:9,class:"hole-ring" }),svgEl("circle",{cx:p.x,cy:p.y,r:3,fill:"#1e40af"})); }
