...
function createServer(packageRoot) {
  return http.createServer((request, response) => {
    ...
    if (pathname === "/__embed-scroll-test.html") {
      ...
      const iframeHeight = requestUrl.searchParams.get("fluid") === "1" ? "100vh" : "500px";
      let srcPath;
      ...
      const html = `<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="data:,"><style>html,body{margin:0;scroll-behavior:auto} .spacer{height:300px} iframe{display:block;width:100%;height:${iframeHeight};border:0} .after{height:1200px}</style><div class="spacer"></div><iframe id="activity" title="embedded activity"></iframe><div class="after"></div><script>const values={"cmi.core.lesson_status":"not attempted"};window.API={LMSInitialize:()=>"true",LMSGetValue:(key)=>values[key]||"",LMSSetValue:(key,value)=>(values[key]=String(value),"true"),LMSCommit:()=>"true",LMSFinish:()=>"true",LMSGetLastError:()=>"0",LMSGetErrorString:()=>"No error",LMSGetDiagnostic:()=>""};const activityFrame=document.getElementById("activity");let panelLockY=null;let panelLockTimer=null;let panelRootOverflow="";let panelBodyOverflow="";window.__panelLockMessages=0;const releasePanelLock=()=>{if(panelLockTimer)clearTimeout(panelLockTimer);panelLockTimer=setTimeout(()=>{document.documentElement.style.overflow=panelRootOverflow;document.body.style.overflow=panelBodyOverflow;document.documentElement.style.overscrollBehavior="";document.body.style.overscrollBehavior="";panelLockY=null;panelLockTimer=null},450)};window.addEventListener("message",event=>{if(event.source!==activityFrame.contentWindow||event.origin!==location.origin||event.data?.source!=="simlab"||event.data?.type!=="panel-gesture")return;window.__panelLockMessages++;if(event.data.phase==="start"){panelLockY=scrollY;panelRootOverflow=document.documentElement.style.overflow;panelBodyOverflow=document.body.style.overflow;document.documentElement.style.overflow="hidden";document.body.style.overflow="hidden";document.documentElement.style.overscrollBehavior="none";document.body.style.overscrollBehavior="none";scrollTo(0,panelLockY)}else if(event.data.phase==="end"){releasePanelLock()}});window.addEventListener("scroll",()=>{if(panelLockY!=null)scrollTo(0,panelLockY)},{passive:true});activityFrame.src=${JSON.stringify(src)};<\/script>`;
