(function (root, factory) {
  ...
  const RULER_EDGE_MIN_GAP_PX = 6;
  const RULER_EDGE_MAX_GAP_PX = 44;
  ...
  function validPlacement(value, task) {
    return Boolean(value && value.task === task && ["pointer", "keyboard"].includes(value.mode) &&
      ...
      finite(value.edgeGapPx) && value.edgeGapPx >= RULER_EDGE_MIN_GAP_PX && value.edgeGapPx <= RULER_EDGE_MAX_GAP_PX);
  }
  function totalPlacementValid(value) { return validPlacement(value, "total"); }
  function gapEvidenceValid(value, task, reading) {
    return Boolean(validPlacement(value, task) && value.usedWhileValid === true && finite(value.readingM) &&
  ...
  function scoreAttempt(answer) {
    ...
    const process = {
      setup: evidence.setupCompleted === true && answer.frequencyActivelySelected === true ? 4 : 0,
  ...
  return {
    ...
    MIN_MEANINGFUL_MOVE_NORM, ZERO_ALIGNMENT_TOLERANCE_PX, RULER_EDGE_MIN_GAP_PX,
    RULER_EDGE_MAX_GAP_PX, TOTAL_KEYS, GAP_KEYS, CONCEPT_ANSWERS, distanceTolerance,
    within, validPlacement, totalPlacementValid, gapEvidenceValid, ratioTarget, scoreRatio,
