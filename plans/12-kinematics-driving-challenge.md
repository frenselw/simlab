...
- 各區之間有不計分過渡段；
- 每區分別評分，各佔 5 分；
- 三區不要求保持相同速度，只要求各自速度穩定；
- 坡度固定，選對力度後可保持按住至區域完結，不需要微調。

重點：

- 同一運動規律在不同外界條件下需要不同操作；
- 踏板方向及力度要配合坡度；
- 進階操作來自路段之間的選擇，不來自區內高頻切換。
...
SLOPE_ANGLES_DEG = [+3.50, +4.68, -4.34]
...
- 三個校準坡度分別對應中油門、油門踩盡及輕煞車，讓學生把坡度與力度結合；
...
- 第 4 關只需在路段轉換時選擇中油門、油門踩盡及輕煞車；每區內保持同一力度可取得滿分；
...
`physicsVersion = 5` 包含速度相依阻力、三段踏板反應及 boundary replay 規則；`levelSetVersion = 5` 包含唯一策略平路關、三段坡度關及重新排序的混合路線。早期 pre-release 的 version 1–4 snapshot 不作隱式遷移，decoder 必須 fail closed，測試亦要覆蓋舊 physics 及舊 level-set rejection。
