(function (root, factory) {
  ...
  function semantic(state, problem) {
    ...
    if (state.part3.selectedCandidateKey !== null && !Model.validObservations(problem.part3.initialView, state.part3.observations)) return null;
    return { balanced, enoughAngle, p1Complete: balanced && state.part1.markX !== null, p2Complete: Boolean(state.part2.mark), p3Complete: state.part3.selectedCandidateKey !== null, hangKeys, lineKeys };
  }
  ...
  function selectPart3(state, key) { if (!Generator.LABELS.includes(key) || !state || !Model.validObservations(Generator.generate(state.seed).part3.initialView, state.part3.observations)) return null; return mutate(state, "part3", (next) => { next.part3.selectedCandidateKey = key; }); }
  ...
  return { VERSION, initial, variantFor, validate, encode, decode, migrateV1, release, markPart1, settleHole, traceVertical, recordLine, markPart2, setView, selectPart3, allComplete, switchPart, enterCheck, resetPart, makeReview, fromReview, lifecyclePolicy };
});
