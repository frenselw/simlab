(function (root, factory) {
  ...
  function settleHole(state, key) { const problem = state && Generator.generate(state.seed); if (!problem?.part2.holes.some((x) => x.key === key) || state.part2.activeHoleKey) return null; return mutate(state, "part2", (next) => { if (!next.part2.hangRecords.some((x) => x.holeKey === key)) next.part2.hangRecords.push({ holeKey: key }); next.part2.activeHoleKey = key; }); }
  ...
  function recordLine(state, line) { if (!state?.part2?.activeHoleKey || line?.holeKey !== state.part2.activeHoleKey) return null; const problem=Generator.generate(state.seed),hole=problem.part2.holes.find((item)=>item.key===line.holeKey);if(!Model.lineRecordable(line,hole,problem.part2.centre,problem.part2.size))return null;return mutate(state, "part2", (next) => { next.part2.lines = next.part2.lines.filter((x) => x.holeKey !== line.holeKey); next.part2.lines.push(clone(line)); }); }
