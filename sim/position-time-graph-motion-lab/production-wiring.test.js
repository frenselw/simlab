...
assert.match(indexSource, /<div id="graphTouchPreviewHost" class="graph-magnifier-host" aria-hidden="true" hidden><\/div>/, "touch magnifier uses an absolute overlay outside the source graph SVG");
...
assert.match(stylesSource, /\.graph-magnifier\s*\{[^}]*width:\s*100%[^}]*height:\s*100%[^}]*pointer-events:\s*none/s, "real graph magnifier cannot intercept dragging or outer-page gestures");
assert.match(stylesSource, /\.graph-magnifier-host\s*\{[^}]*position:\s*absolute[^}]*width:\s*min\(52vw, 11\.5rem\)[^}]*pointer-events:\s*none/s, "magnifier overlay has readable CSS dimensions without changing graph layout");
assert.match(source, /dom\.graphLayer\.innerHTML[\s\S]*graphMagnifierSource\(\)/, "magnifier clones the current rendered x-t graph rather than generating a summary graphic");
...
assert.ok(magnifierMarkup.includes('class="graph-magnifier"'), "touch graph drag immediately shows a real SVG magnifier");
