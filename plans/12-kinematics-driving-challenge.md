...
- 學生從輕、中或盡油門選定一個力度，由計分區起點持續按住即可製造勻加速；
- 三種固定油門力度都可取得滿分，不需要在路段中途切換。
...
- 道路為平地；
- 不指定減速度大小；
- 學生從輕、中或盡煞車選定一個力度，由計分區起點持續按住，令 `v–t` 圖形成明顯向下而且近似筆直的線；
- 三種固定煞車力度都可取得滿分，不需要在路段中途切換；
- 車必須保持向前並完成整個區域；
- 在區域中過早停車不算成功的勻減速；
- 突然重煞後長時間慢速滑行不算勻減速。
...
a_pedal(空檔) = 0
a_pedal(油門輕／中／盡) = +(0.50／0.75／1.00) m/s²
a_pedal(煞車輕／中／盡) = -(0.50／0.75／1.00) m/s²
```
...
- 平路空檔 → 勻速；
- 平路持續按住任何一個固定油門力度 → 勻加速；
- 平路持續按住任何一個固定煞車力度 → 勻減速；
- 固定坡度配合一個合適的固定踏板力度 → 近似勻速。

因此學生需要選擇力度，但不需要因車速改變而持續微調；平路勻加速／勻減速關只檢查所產生的運動規律，不指定唯一力度。
...
```text
GRAVITY_M_S2 = 9.81
THROTTLE_ACCELERATION_M_S2 = [0.50, 0.75, 1.00]
BRAKE_ACCELERATION_M_S2 = [0.50, 0.75, 1.00]

SLOPE_ANGLES_DEG = [+2.92, +5.85, -4.38]
...
- 每一個固定力度在固定坡度上產生固定加速度；
- 三個校準坡度分別對應輕油門、盡油門及中煞車，讓學生從坡度選擇適當力度；
...
- 第 1 關全程空檔可取得滿分；
- 第 2 關選定任一固定油門力度並全程長按均可取得滿分；
- 第 3 關選定任一固定煞車力度並全程長按均可取得滿分；
- 第 4 關只需在路段轉換時選擇輕油門、盡油門及中煞車；每區內保持同一力度可取得滿分；
- 第 5 關只需在路段轉換時於空檔、固定油門及固定煞車之間切換；
- 錯誤的固定輸入不能在勻加速或勻減速關卡取得滿分；
- 容差剛內／剛外的序列必須可人工構造並測試；
- 320 CSS-pixel 手機、60 Hz、120 Hz 及低 frame rate 的權威結果相同。

若第 2 或第 3 關的任一固定力度不能取得滿分，或第 4、5 關要求區內高頻切換，必須調整踏板加速度、坡度、初速、區域長度或容差。
...
`physicsVersion = 4` 包含三段油門／煞車、無速度相依阻力的教學模型及 boundary replay 規則；`levelSetVersion = 4` 包含三段坡度選擇關及相應可解性校準。早期 pre-release 的 version 1／2／3 snapshot 不作隱式遷移，decoder 必須 fail closed，測試亦要覆蓋舊 physics 及舊 level-set rejection。
...
- 每區在最短可評時間後仍有足夠距離；
- 第 1 關空檔、第 2 關三種固定油門、第 3 關三種固定煞車均可完成路段並取得滿分；
...
- 油門與煞車不能同時疊加。
- 第 2、3 關的三種力度各自都有不需中途切換的滿分策略。
- 重試不扣分，不會在確認前刪除已記錄 run。
- 每關後有無數字回放、質性回饋及改善選擇。
- 每個固定踏板力度在固定坡度上產生固定加速度。
- 車停止後不倒後，第一版沒有負速度。
