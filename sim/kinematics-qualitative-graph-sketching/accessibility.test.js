...
assert.doesNotMatch(html, /id="checkGraphButton"|檢查畫法|取得作圖提示|只提供修改建議/);
assert.doesNotMatch(html, /id="skipButton"|稍後再做/);
assert.match(html, /id="stageCompletenessHint"[^>]*class="graph-completeness-hint is-hidden"[\s\S]*?>圖線未畫完</);
assert.ok((html.match(/class="minor-grid"/g) || []).length >= 8);
...
assert.match(css, /grid-template-rows:\s*minmax\(13rem,\s*44dvh\)/);
