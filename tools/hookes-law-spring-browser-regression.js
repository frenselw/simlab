...
async function navigate(cdp, url, embedded = false) {
  await evaluate(cdp, embedded
    ? "(() => { const frame=document.getElementById('activity'); if (frame?.contentWindow) frame.contentWindow.__hookesLawApp = undefined; })()"
    : "window.__hookesLawApp = undefined");
  await cdp.send("Page.navigate", { url });
  await delay(250);
  const expectedUrl = JSON.stringify(url);
  for (let attempt = 0; attempt < 160; attempt += 1) {
    const ready = await evaluate(cdp, embedded
      ? `(() => { const frame=document.getElementById('activity'); return Boolean(window.location.href === ${expectedUrl} && frame?.contentWindow?.__hookesLawApp && frame.contentDocument?.readyState === 'complete'); })()`
      : `Boolean(window.__hookesLawApp && document.readyState === 'complete' && window.location.href === ${expectedUrl})`);
    if (ready) return;
    await delay(50);
  }
...
async function runDirectFlow(cdp, baseUrl, launchPath, label) {
  ...
  const initial = await evaluate(cdp, `(() => { const d=document; return {
    presentation:window.__hookesLawApp.getPresentation(), resultHidden:d.getElementById('resultPanel').classList.contains('is-hidden'),
    ...
    legacyDebugControls:d.querySelectorAll('#debugPanel,#debugCompleteInvestigation,#debugCompleteModel').length,
  ...
  assert.equal(initial.legacyDebugControls, 0, `${label}: legacy debug query exposes no debug controls`);
  ...
  const switchedSpring = await evaluate(cdp, "(() => ({active:window.__hookesLawApp.getState().activeSpring,pressed:[...document.querySelectorAll('[data-action=\\\"spring-tab\\\"]')].map((node)=>node.getAttribute('aria-pressed'))}))()");
  ...
  const zeroAfter = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),spring=window.__hookesLawApp.getScenario().springs[state.activeSpring];return {value:state.working.zeroDraftM,target:spring.naturalLengthM}; })()");
  ...
  const cursorAfter = await evaluate(cdp, "(() => { const debug=window.__hookesLawApp,state=debug.getState(),spring=debug.getScenario().springs[state.activeSpring],target=window.HookesLawModel.endpointM(spring.naturalLengthM,window.HookesLawScoring.forceByKey[state.activeLoadKey],spring.kNPerM);return {value:state.working.cursorDraftM,target}; })()");
  ...
  await evaluate(cdp, `(() => { const answer=${fixtureExpression(123)}; window.__hookesLawApp.routeAttempt({state:'draft',snapshot:{version:1,activity:'${slug}',kind:'draft',answer}}); })()`);
  await evaluate(cdp, `(() => { const answer=${fixtureExpression(123)},G=window.HookesLawGenerator,P=window.HookesLawPersistence,scenario=G.generateScenario({seed:answer.seed}),review=P.transitions.setPhase(answer,'review',scenario),edited=P.transitions.editSection(review,'investigate',scenario); window.__hookesLawApp.routeAttempt({state:'draft',snapshot:{version:1,activity:'${slug}',kind:'draft',answer:edited}}); })()`);
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'investigate'", `${label}: measurement review edit could not reopen the investigation phase`);
  await clickDirect(cdp, "[data-action='select-load'][data-load='F1']");
  await clickDirect(cdp, "[data-action='attach-load']");
  ...
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'model'", `${label}: model semantic regression could not open the model phase`);
  const modelBeforeNoOp = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),key=state.activeSpring; return {key,handle:state.models[key].handleExtensionM,predictions:JSON.stringify(state.predictions),design:JSON.stringify(state.design),fromReview:state.fromReview}; })()");
  await pressKey(cdp, "#modelDrag", "ArrowUp");
  await delay(45);
  const modelAfterVertical = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),key=state.activeSpring; return {handle:state.models[key].handleExtensionM,predictions:JSON.stringify(state.predictions),design:JSON.stringify(state.design),fromReview:state.fromReview,force:window.__hookesLawApp.interactionEvidence().modelDraftForceN,status:document.getElementById('modelStatus').textContent,live:document.getElementById('liveRegion').textContent}; })()");
  ...
  await delay(45);
  const modelAfterHorizontal = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),key=state.activeSpring,evidence=window.__hookesLawApp.interactionEvidence(); return {handle:state.models[key].handleExtensionM,draft:evidence.modelDraftM,move:evidence.modelMoveM,predictions:state.predictions.filter(Boolean).length,design:state.design,fromReview:state.fromReview,status:document.getElementById('modelStatus').textContent,live:document.getElementById('liveRegion').textContent}; })()");
  ...
  await delay(80);
  const review = await evaluate(cdp, `(() => ({ phase:window.__hookesLawApp.getState().phase, hidden:document.getElementById('resultPanel').classList.contains('is-hidden'), text:document.getElementById('controlPanel').textContent }))()`);
  ...
  for (const testCase of [
    ...
    await evaluate(cdp, `(() => { const answer=${fixtureExpression(123, testCase.modelK)}; window.__hookesLawApp.routeAttempt({state:'draft',snapshot:{version:1,activity:'${slug}',kind:'draft',answer}}); })()`);
    await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'design'", `${label}: review geometry fixture did not load`);
    await clickDirect(cdp, "[data-action='to-review']");
    await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'review'", `${label}: review geometry fixture did not open review`);
    const geometry = await evaluate(cdp, `(() => { const svg=document.querySelector('#reviewSummary svg[data-review-model="${testCase.springKey}"]'),line=svg?.querySelector('[data-review-line="learner-model"]'),left=34,top=14,width=208,height=106,maxX=.18,maxF=4,x2=Number(line?.getAttribute('x2')),y2=Number(line?.getAttribute('y2')),endX=(x2-left)/width*maxX,endF=(top+height-y2)/height*maxF; return {endX,endF,slope:endF/endX,model:window.__hookesLawApp.getState().models.${testCase.springKey}.handleExtensionM}; })()`);
  ...
  await evaluate(cdp, `(() => { const answer=${fixtureExpression(123)}; window.__hookesLawApp.routeAttempt({state:'draft',snapshot:{version:1,activity:'${slug}',kind:'draft',answer}}); })()`);
  await clickDirect(cdp, "[data-action='to-review']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'review'", `${label}: original review fixture could not be restored after geometry checks`);
  await clickDirect(cdp, "[data-action='edit-section'][data-edit-phase='model']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'model'", `${label}: review model edit did not reopen the model phase`);
  ...
  await evaluate(cdp, `(() => { const answer=${fixtureExpression(123)}; window.__hookesLawApp.routeAttempt({state:'draft',snapshot:{version:1,activity:'${slug}',kind:'draft',answer}}); })()`);
  await clickDirect(cdp, "[data-action='to-review']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'review'", `${label}: review fixture could not be restored after focus check`);

  await evaluate(cdp, "document.querySelector('[data-action=submit]').click()");
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (await evaluate(cdp, "window.__hookesLawApp.getPresentation() === 'submitted-success'")) break;
    await delay(25);
  }
  const result = await evaluate(cdp, `(() => ({ presentation:window.__hookesLawApp.getPresentation(), hidden:document.getElementById('resultPanel').classList.contains('is-hidden'), text:document.getElementById('resultPanel').textContent, score:window.__hookesLawApp.getResult()?.score }))()`);
...
async function runFirstLoadDependencyLock(cdp, baseUrl, launchPath, label, dependencyFile) {
  ...
  try {
    ...
    const outcome = await evaluate(cdp, "(() => { const panelIds=['investigatePanel','modelPanel','predictPanel','designPanel','reviewPanel'],answerControls=[...document.querySelectorAll(panelIds.map((id)=>'#'+id+' [data-action]').join(','))]; return {presentation:window.__hookesLawApp?.getPresentation?.()||null,state:window.__hookesLawApp?.getState?.()||null,result:window.__hookesLawApp?.getResult?.()||null,technicalHidden:document.getElementById('technicalPanel')?.classList.contains('is-hidden')??true,answerPanelsHidden:panelIds.every((id)=>document.getElementById(id)?.classList.contains('is-hidden')),resultHidden:document.getElementById('resultPanel')?.classList.contains('is-hidden')??true,resultText:document.getElementById('resultPanel')?.textContent||'',visibleEnabledAnswerControls:answerControls.filter((node)=>!node.disabled&&!node.closest('.is-hidden')).length,stageChildCount:document.getElementById('stageSvg')?.childElementCount??-1,bootErrors:window.__hookesLawBootErrors||[]}; })()");
...
async function completeLearnerPath(cdp, baseUrl, launchPath, label, keyboard = false) {
  ...
  for (const springKey of ["A", "B"]) {
    ...
    if (keyboard) {
      if (springKey === "A") {
        const before = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),rect=document.getElementById('zeroDrag').getBoundingClientRect(); return {value:state.working.zeroDraftM,top:rect.top}; })()");
        await pressKey(cdp, "#zeroDrag", "ArrowUp");
        const afterUp = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),rect=document.getElementById('zeroDrag').getBoundingClientRect(); return {value:state.working.zeroDraftM,top:rect.top}; })()");
    ...
    await clickDirect(cdp, "[data-action='record-calibration']");
    await waitUntil(cdp, `Boolean(window.__hookesLawApp.getState().calibrations.${springKey})`, `${label}: ${springKey} calibration did not save`);
    for (const loadKey of ["F1", "F2", "F3"]) {
      ...
      if (keyboard) {
        if (springKey === "A" && loadKey === "F1") {
          await pressKey(cdp, "#cursorDrag", "ArrowDown", 6);
          const before = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),rect=document.getElementById('cursorDrag').getBoundingClientRect(); return {value:state.working.cursorDraftM,top:rect.top,label:document.getElementById('cursorDrag').getAttribute('aria-label')}; })()");
          ...
          const afterUp = await evaluate(cdp, "(() => { const state=window.__hookesLawApp.getState(),rect=document.getElementById('cursorDrag').getBoundingClientRect(); return {value:state.working.cursorDraftM,top:rect.top}; })()");
      ...
      await clickDirect(cdp, "[data-action='record-measurement']");
      await waitUntil(cdp, `Boolean(window.__hookesLawApp.getState().measurements.${springKey}.${loadKey})`, `${label}: ${springKey}/${loadKey} measurement did not save`);
    }
  }
  await clickDirect(cdp, "[data-action='to-model']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'model'", `${label}: model phase did not open`);
  ...
  for (const springKey of ["A", "B"]) {
    ...
    const modelEvidence = await evaluate(cdp, "window.__hookesLawApp.interactionEvidence()");
    assert.ok(modelEvidence.modelDraftForceN > 2.5, `${label}: model line control can move upward through the full graph`);
    await waitUntil(cdp, `Boolean(window.__hookesLawApp.getState().models.${springKey})`, `${label}: ${springKey} model did not save`);
  }
  await clickDirect(cdp, "[data-action='to-predict']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'predict'", `${label}: predict phase did not open`);
  const predictionStage = await evaluate(cdp, `(() => {
    const d=document, state=window.__hookesLawApp.getState(),
      stageText=[...d.querySelectorAll('#stageSvg text')].map((node)=>node.textContent).filter(Boolean),
    ...
    return {phase:state.phase, draftM:window.__hookesLawApp.interactionEvidence().predictionDraftM,
  ...
  if (keyboard) {
    const before = await evaluate(cdp, "(() => { const evidence=window.__hookesLawApp.interactionEvidence(),rect=document.getElementById('predictionDrag').getBoundingClientRect(); return {value:evidence.predictionDraftM,top:rect.top}; })()");
    await pressKey(cdp, "#predictionDrag", "ArrowDown");
    const afterDown = await evaluate(cdp, "(() => { const evidence=window.__hookesLawApp.interactionEvidence(),rect=document.getElementById('predictionDrag').getBoundingClientRect(); return {value:evidence.predictionDraftM,top:rect.top}; })()");
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    if (keyboard) await pressKey(cdp, "#predictionDrag", "ArrowDown", 6);
      ...
      const exactTarget = await evaluate(cdp, `(() => {
        const debug=window.__hookesLawApp,state=debug.getState(),spec=debug.getScenario().predictions[state.activePredictionIndex],
      ...
      const exactPrediction = await evaluate(cdp, "window.__hookesLawApp.getState().predictions[0]?.extensionM");
    ...
    await waitUntil(cdp, `Boolean(window.__hookesLawApp.getState().predictions[${index}])`, `${label}: prediction ${index + 1} did not save`);
  }
  const completedPredictionPhase = await evaluate(cdp, "({phase:window.__hookesLawApp.getState().phase,toDesignDisabled:document.getElementById(\"toDesign\").disabled})");
  ...
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'design'", `${label}: design phase did not open`);
  ...
  const recordedPredictions = await evaluate(cdp, "JSON.stringify(window.__hookesLawApp.getState().predictions)");
  await clickDirect(cdp, "[data-action='navigate-phase'][data-phase='predict']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'predict'", `${label}: design phase could not return to prediction`);
  const returnedFromDesign = await evaluate(cdp, "({phase:window.__hookesLawApp.getState().phase,predictions:JSON.stringify(window.__hookesLawApp.getState().predictions),design:window.__hookesLawApp.getState().design})");
  ...
  await clickDirect(cdp, "[data-action='navigate-phase'][data-phase='model']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'model'", `${label}: prediction phase could not return to model`);
  const returnedToModel = await evaluate(cdp, "({predictions:JSON.stringify(window.__hookesLawApp.getState().predictions),design:window.__hookesLawApp.getState().design,fromReview:window.__hookesLawApp.getState().fromReview})");
  ...
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'predict'", `${label}: model phase could not return to prediction`);
  const returnedToPredict = await evaluate(cdp, "JSON.stringify(window.__hookesLawApp.getState().predictions)");
  assert.equal(returnedToPredict, recordedPredictions, `${label}: returning to prediction keeps recorded results unchanged`);
  await clickDirect(cdp, "#toDesign");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'design'", `${label}: prediction phase could not return to design`);
  ...
  await clickDirect(cdp, "[data-action='to-review']");
  await waitUntil(cdp, "window.__hookesLawApp.getState().phase === 'review'", `${label}: review did not open`);
  const review = await evaluate(cdp, "({phase:window.__hookesLawApp.getState().phase,hidden:document.getElementById('resultPanel').classList.contains('is-hidden'),controlText:document.getElementById('controlPanel')?.textContent||'',stageText:[...document.querySelectorAll('#stageSvg text')].map((node)=>node.textContent)})");
  ...
  await waitUntil(cdp, "['submitted-success','submitted-committed'].includes(window.__hookesLawApp.getPresentation())", `${label}: full learner path did not submit`);
  const result = await evaluate(cdp, "({presentation:window.__hookesLawApp.getPresentation(),score:window.__hookesLawApp.getResult()?.score})");
...
async function runTouchMatrix(cdp, baseUrl, launchPath, label) {
  ...
  const dragAfter = await evaluate(cdp, `(() => { const f=document.getElementById('activity'),d=f.contentDocument,w=f.contentWindow,p=d.getElementById('controlPanel');return {events:w.__touchEvents,host:scrollY,panel:p.scrollTop,zero:w.__hookesLawApp?.getState().working.zeroDraftM||null,target:d.elementFromPoint(${dragPoint.x - (await iframeOffset(cdp)).left},${dragPoint.y - (await iframeOffset(cdp)).top})?.id||''}; })()`);
...
async function main() {
  ...
  try {
    ...
    summary = `Hooke's law browser regression passed: ${sourceMissingScorm}; ${sourceMissingFlow}; ${packageMissingScorm}; ${packageMissingFlow}; ${sourceMissingLocal.join("; ")}; ${packageMissingLocal.join("; ")}; ${sourceDirect}; ${packageDirect}; ${sourcePointer}; ${packagePointer}; ${sourceKeyboard}; ${packageKeyboard}; ${sourceTouch}; ${packageTouch}`;
  } catch (error) {
