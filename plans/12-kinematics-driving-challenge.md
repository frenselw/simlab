...
- 每一個 segment 入口都在整個馬路闊度畫出高對比分界線；黃色實線直接標示「由此開始計分」，白色虛線直接標示「轉換區／不計分」；
- 白色轉換線旁顯示「下一區：保持……」，黃色計分線旁顯示該區目標；不能只使用「開始」「準備」等未說明對象的詞語；
- 舞台左上角在計分區顯示「計分中：保持……」，在過渡區顯示「轉換區（不計分）｜下一區：保持……」；
- 分界線、明確文字及路牌同時存在，不能只靠顏色或路牌判斷切換位置；
...
- 第 5 關每個 segment 入口都有跨越整個馬路闊度的分界線；文字直接說明「由此開始計分」或「轉換區（不計分）」及下一區目標。
- 預覽圖沒有任何數字刻度或數值。
