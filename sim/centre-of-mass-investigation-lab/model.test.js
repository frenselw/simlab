...
for(const hole of p.part2.holes){const swing=M.createSwing(p.part2,hole,0);assert.equal(swing.inertia,swing.inertiaCm+swing.mass*swing.distance*swing.distance,"parallel-axis theorem Ip = Icm + M d^2");assert.ok(swing.damping>0&&Number.isFinite(swing.damping));let time=0;while(time<6&&!M.stepSwing(swing,1/60))time+=1/60;assert.ok(time>=1.5&&time<=4.5,`finite visibly damped settle: ${time}`);assert.ok(Math.abs(swing.angle-swing.target)<1e-12);const pivot=M.transform(hole,{x:0,y:0,angle:swing.target}),com=M.transform(p.part2.centre,{x:0,y:0,angle:swing.target});assert.ok(Math.abs(com.x-pivot.x)<1e-12&&com.y>pivot.y,"analytic equilibrium puts COM vertically below pivot");}
