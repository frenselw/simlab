...
- stage 顯示清楚的「安全承載設計・找出最大安全負載」任務字句；縱軸 \(0\) 刻度與固定端／天花板處於同一水平位置，並畫出已選彈簧、紅色 \(x_{\max}\) 安全上限線及學生模型預測的末端位置；
- 未選擇彈簧時，提示文字依安全上限線的實際位置放在虛線下方，與安全上限標籤及刻度保持間隔，不可固定在可能重合的高度；
- 負載由彈簧下方垂直堆疊的細小灰色方塊表示；開始只有一粒，每增加一個 module 就多一粒，整體負載向下延長；不另畫底部橫向模組圖示，也不把 \(\mathrm{N}\) 文字貼在方塊後方；
