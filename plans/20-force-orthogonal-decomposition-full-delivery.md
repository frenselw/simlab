...
- 學生任務：在三個固定情境中，依序畫兩條分解方向、從原力作兩條垂線、畫兩個分力、標示 θ，再以三角函數表示兩個分力大小；可查看作答完整狀態及返回修改，最後在總覽提交後才查看評核回饋。
...
- 公式：兩個分力函數各 10 分；缺填及錯填只影響相應的 10 分項。最終評分按每支分力自身的軸向和合法 θ 獨立判斷，其他作圖項的錯誤不連帶扣公式分；第三題仍按固定 Gₓ／Gᵧ 名稱判斷。未能確定軸向、θ 或欠缺該支分力時得 0 分並標示「未能判斷」，不誤稱 sin／cos 選錯。公式卡只收集學生答案，練習期間沒有公式檢查按鈕或即時對錯提示；可編輯條件仍要求完整幾何，不因逐項評分而放寬。
...
## Milestone evidence log（2026-09-18 歷史結果）
...
## Final verification record（2026-09-18 歷史結果）
...
# 歷史管理與驗證紀錄（2026-09-18）

以下保留當時的工作段、巡查、工作樹整合及測試證據。所有「active」「待驗收」「繼續 heartbeat」與協作指令均為歷史敘述，不表示目前任務狀態，也不授權重新啟動工作者或監控。現行功能與目錄狀態以文件開頭及生產計劃為準；歷史測試結果不可自動沿用至後續版本。
