(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function beginDrag(event,target,kind,onMove,onEnd=()=>{}) { if (gesture || locked || event.button>0) return; event.preventDefault();target.setPointerCapture(event.pointerId);const start=clientPoint(event);gesture={id:event.pointerId,target,kind,last:start,onMove,onEnd,plateGrabLocal:kind==="plate"?svgToLocal(start):null,baselineState:Persistence.decode(Persistence.encode(committedState)),baselineSupport:supportX,baselinePose:{...platePose}};target.classList.add("active-direct");if(event.pointerType!=="mouse"&&state?.phase==="part2")showPreview(event,kind);target.addEventListener("pointermove",moveGesture);target.addEventListener("pointerup",endGesture,{once:true});target.addEventListener("pointercancel",cancelGesture,{once:true});target.addEventListener("lostpointercapture",lostGesture,{once:true}); }
    ...
    function previewFocus() { const kind = dom.preview.dataset.kind, last = gesture?.last;
      if (kind === "plate" && gesture?.plateGrabLocal) return localToSvg(gesture.plateGrabLocal);
      ...
      if (kind === "draw" && drawGhost) return drawGhost.b;
    ...
    function showPreview(event,kind){if(state?.phase!=="part2")return;const rect=dom.stage.getBoundingClientRect(),corner=UiPolicy.previewCorner({x:event.clientX,y:event.clientY},{left:rect.left,top:rect.top,width:rect.width,height:rect.height});dom.preview.className=`preview ${corner}`;dom.preview.dataset.kind=kind;renderSvgPreview(previewFocus());}
