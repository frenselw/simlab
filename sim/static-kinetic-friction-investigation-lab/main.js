(function (root, factory) {
  ...
  const PHASE_LABELS = Object.freeze({ balance: "A 水平受力", experiment: "B 實驗", analysis: "C 圖上標記", predict: "D 預測", review: "檢查與提交" });
  ...
  function createTechnicalApp(error) {
    ...
    function lock() {
      ...
      const title = document.getElementById("technicalTitle"); if (title) title.textContent = "活動暫時無法使用";
  ...
  function createController() {
    ...
    let balanceFrictionCleared = false;
    ...
    function navigateToPhase(phase) {
      ...
      hideRedoExperimentConfirmation();
      ...
      saveDraft();
      announce(`已切換到 ${({ balance: "A 水平受力", experiment: "B 實驗", analysis: "C 圖上標記", predict: "D 預測" })[phase] || phase}`);
    }
    ...
    function syncBalanceDrawings() {
      ...
      balanceFrictionCleared = source?.learnerForce?.committed === true && source.learnerForce.frictionType === "none";
      balanceDrawings = {
    ...
    function balanceCurrentForceN() {
      ...
      const rawForceN = clamp((balanceForceEndpointX - balanceComX()) / 18, -12, 12);
      return Math.round(rawForceN * 10) / 10;
    }
    ...
    function renderStageCoach() {
      ...
      if (state.phase === "balance") {
        if (state.fromReview) {
          ...
          step = "A"; title = "正在修改 Part A 答案"; text = target === "zero-force" ? "修改 A1：沒有水平外力時，摩擦力是否存在？" : target === "static-case" ? "修改 A2：重新畫出指定拉力，並判斷是否需要畫出摩擦力箭頭。" : "修改 A3：重新輸入你估計的最大靜摩擦力。";
        } else if (!state.balance.zeroForce) {
          step = "A1"; title = "先判斷沒有水平外力時的摩擦力"; text = "請選擇摩擦力的類型、方向和大小。";
        } else if (balanceStaticInteractionActive()) {
          step = "A2"; title = "先畫出指定拉力，再判斷是否需要畫出摩擦力"; text = balanceDrawMode === "friction" ? "請由物體中央畫出你判斷的摩擦力；若認為沒有摩擦力，請按「不畫摩擦力」。" : "請由物體中央畫出指定拉力；箭頭方向和長度代表你的答案。";
          ...
          step = "A3✓"; title = "已找到開始滑動的臨界拉力"; text = "根據你觀察到的臨界拉力，填寫最大靜摩擦力估計。";
        } else {
          step = "A✓"; title = "Part A 三個概念任務已完成"; text = "可以用上方任務列自由切換到 Part B、C 或 D；之後仍可返回修改 Part A。";
        }
      } else if (state.phase === "experiment") {
        step = state.trial ? "B✓" : recorder?.running && experimentAutoKineticHold ? "B2" : "B1";
        if (state.fromReview) {
          title = "正在修改 Part B 實驗"; text = "按「重新開始」會清除舊記錄，並開始新的 30 秒記錄。";
        } else if (state.trial) {
          title = "B 記錄已保存"; text = "可以前往 Part C 分析同一張 F拉–t 圖；亦可以自由切換其他任務。";
        } else if (experimentTimedOut) {
          title = "30 秒記錄已結束"; text = "請按「重新開始」，再在 30 秒內完成一份可分析的實驗記錄。";
          ...
          title = "先按「開始 30 秒記錄」"; text = "開始後按住物體中央的小圓點向右拖動，逐漸增加拉力，直到物體開始滑動。";
        }
    ...
    function abortExperimentRecording(message = "已中止未完成的 B 記錄；切換任務後可重新開始。") {
      ...
      experimentInputClockReady = false;
      experimentPendingInputs = [];
      experimentStatusOverride = message || null;
      stopLoop();
    ...
    function balanceMotionStatusText(forceN = balanceCurrentForceN()) {
      ...
      if (Math.abs(velocity) <= 1e-5) return Math.abs(forceN) > 1e-4 ? "拉力未超過最大靜摩擦力，物體仍然靜止；可以繼續改變拉力。" : "物體目前靜止；按住物體中央的拉力箭頭，即可向左或向右施力。";
      if (Math.abs(acceleration) < .035) return "物體正在近似勻速運動；拉力箭頭仍可即時向左或向右改變。";
      if (velocity * acceleration > 0) return "物體正在加速；拉力箭頭仍可即時向左或向右改變。";
      return "物體正在減速；向運動的相反方向施力，可使物體停下並反向運動。";
    }
    ...
    function experimentStatusText() {
      if (experimentStatusOverride) return experimentStatusOverride;
      if (state?.trial) return "記錄已保存，可以前往 Part C。";
      if (experimentTimedOut) return "30 秒記錄已結束。請按「重新開始」，再在 30 秒內完成一份可分析的實驗記錄。";
      if (!recorder?.running || !directExperimentState) return "尚未開始記錄。";
      if (experimentAutoKineticHold) return "物體已開始滑動；系統正自動調節拉力，使物體保持接近勻速運動。";
      return "記錄中：逐漸增加拉力。";
    }
    function predictionPullText(spec) {
      const pullN = finite(spec?.pullN);
      return pullN > .01 ? `已知向右拉力：F拉 = ${pullN.toFixed(1)} N` : "沒有水平拉力：F拉 = 0.0 N";
    }
    function predictionPullMarkup(spec) {
      const pullN = finite(spec?.pullN);
      return pullN > .01 ? `已知向右拉力：<var>F</var><sub>拉</sub> = ${pullN.toFixed(1)} N` : "沒有水平拉力：<var>F</var><sub>拉</sub> = 0.0 N";
    }
    function predictionPullLabel(spec) {
      const pullN = finite(spec?.pullN);
      return pullN > .01 ? `已知拉力 ${pullN.toFixed(1)} N` : "沒有水平拉力 0.0 N";
    }
    ...
    function renderApparatus() {
      ...
      const graphMode = state?.phase === "analysis" || Boolean(state?.trial && mayRevealCorrectness(presentation));
      ...
      if (predictionMode) {
        ...
        if ((spec?.pullN || 0) > .01) appendForceArrow(svg, comX, comX + Math.min(180, (spec?.pullN || 0) * scale), comY, "pull-arrow prediction-pull-arrow", "#b91c1c", predictionPullLabel(spec), pullLabelY);
        ...
        setText("predictionReadout", `D${index + 1}：${predictionPullText(spec)}；${spec?.velocityMps > 0 ? `物體正以 ${spec.velocityMps.toFixed(2)} m/s 向右運動` : "物體目前靜止"}；畫出的摩擦力 ${frictionN > .01 ? `${frictionN.toFixed(2)} N` : "0 N"}。`);
      }
      setText("experimentStatus", experimentStatusText());
    }
    function renderBalance() {
      ...
      updateBreakawayReadout(balanceCurrentForceN());
      const attempts = state.balance.breakaway?.attempts || 0;
      setText("breakawayBest", best == null ? "" : `你已觀察到開始滑動的臨界拉力約 ${(best / 100).toFixed(1)} N；可繼續向左或向右試拉。`);
      setText("breakawayAttempts", attempts ? `試拉次數：${attempts}` : "");
      const applied = balanceDrawings.applied;
      const friction = balanceDrawings.friction;
      ...
      const frictionExplicitlyNone = balanceFrictionCleared;
      setText("staticFrictionReadout", friction ? `你的摩擦力：${directionText(friction.direction)} ${(friction.magnitudeCN / 100).toFixed(1)} N（靜摩擦力）` : frictionExplicitlyNone ? "你的摩擦力：沒有摩擦力" : "你的摩擦力：未畫出（若直接保存，將視為沒有摩擦力）");
    ...
    function renderQuality() {
      if (!experimentQuality) return;
      setText("experimentStatus", state?.trial ? "這次記錄符合要求，可以進入 Part C。" : experimentQuality.neutralMessage);
    }
    function stopLoop() {
      if (loop != null) {
      ...
      while (experimentAccumulatorS >= Physics.PHYSICS_DT_S - 1e-9 && directExperimentState.timeS < Measurement.MAX_TRIAL_DURATION_S - 1e-9) {
        ...
        if (nextPhysical.events?.length) for (const event of nextPhysical.events) {
          ...
          if (event.type === "breakaway" && !breakawayAnnounced) {
            ...
            setText("experimentStatus", "物體已開始滑動；系統正自動調節拉力，使物體保持接近勻速運動。");
          }
        }
    ...
    function hideRedoExperimentConfirmation() {
      redoExperimentPending = false;
      q("redoExperimentConfirm")?.classList.add("is-hidden");
    }
    function requestRedoExperiment() {
      if (!state) return false;
      if (state.trial) {
        redoExperimentPending = true;
        q("redoExperimentConfirm")?.classList.remove("is-hidden");
        focusNode(q("[data-action='confirm-redo-experiment']"));
        return true;
      }
      return restartExperimentImmediately();
    }
    ...
    function finalizeExperimentRecording({ timedOut = false } = {}) {
      ...
      if (!stopped.accepted) {
        ...
        experimentPendingInputs = [];
        experimentAutoHoldElapsedS = 0;
        experimentQuality = { valid: false, neutralMessage: stopped.reason === "sensor-overrange" ? "測力計讀數超出量程。請重新開始，並減少拉力。" : "這次記錄未能保存。請重新開始。" };
      ...
      if (quality.valid) {
        ...
        analysisDraft = null;
        saveDraft();
        setText("experimentStatus", "這次記錄符合要求，可以進入 Part C。");
      } else {
        const message = quality.neutralMessage;
    ...
    function renderGraph() {
      ...
      if (mayRevealCorrectness(presentation)) {
        const authority = latestResult?.breakdown?.analysis;
        const candidates = authority?.candidates || Measurement.findCandidateWindows(decoded);
        ...
        addRange(breakawayTimeS - Scoring.BREAKAWAY_TIME_TOLERANCE_S, breakawayTimeS + Scoring.BREAKAWAY_TIME_TOLERANCE_S, "#c2410c", "最大靜摩擦力");
        const traceEndS = decoded.merged.at(-1)?.timeS;
        addRange(breakawayTimeS + Scoring.KINETIC_MARKER_SETTLE_S, traceEndS, "#15803d", "滑動摩擦力");
      }
    ...
    function renderAnalysisTasks() {
      ...
      const rows = ANALYSIS_MARKER_META.map((marker, index) => {
        ...
        const status = dirty ? "・有未保存的修改" : Persistence.analysisTaskComplete(marker.key, state.analysis?.[marker.key]) ? "・已保存" : "";
    ...
    function scoreDetailReason(key, found) {
      if (found?.correct || found?.points === found?.max) return "";
      const missing = [];
      if (key === "zero-force") {
        if (!found.type) missing.push("摩擦力類型"); if (!found.direction) missing.push("方向"); if (!found.magnitude) missing.push("大小");
      ...
      } else if (key === "maximum-static-friction" && Object.hasOwn(found || {}, "toleranceN")) return "估計值尚未保存，或超出容許誤差範圍";
      else if (key === "breakaway") return "沒有保存能清楚顯示物體開始滑動的有效記錄";
      else if (key === "continued-motion") return "物體開始滑動後的記錄時間不足";
      else if (key === "static-friction") return "標記尚未保存，或不在物體仍靜止且拉力逐漸增加的區段";
      else if (key === "maximum-static-friction") return "標記尚未保存，或不在物體剛開始滑動的時刻附近";
      else if (key === "kinetic-friction") return "標記尚未保存，或不在物體開始滑動後近似勻速運動的區段";
      else {
      ...
      return missing.length ? `以下項目未得分：${missing.join("、")}` : "未完成，或答案不符合要求";
    }
    function scoreBreakdownMarkup(result, currentScenario) {
      ...
      const groups = [
        ...
        { key: "experiment", title: "Part B：拉力—時間實驗", items: [["breakaway", "B1 記錄物體開始滑動", 10], ["continued-motion", "B2 記錄滑動後的持續運動", 10]] },
      ...
      return `<section class="score-breakdown" aria-label="逐部分得分及扣分"><h3>各部分得分及扣分</h3>${groups.map((group) => {
        ...
        const rows = group.items.map(([key, label, fallbackMax], index) => {
          ...
          const reason = scoreDetailReason(key, { ...found, points, max });
          const status = lost === 0 ? "滿分" : `扣 ${lost} 分：${reason}`;
    ...
    function renderResult() {
      ...
      const explanation = scenario ? `<details><summary>本次模擬的參考值</summary><p>物體質量 ${scenario.massKg.toFixed(1)} kg；最大靜摩擦力約 ${scenario.staticLimitMeanN.toFixed(2)} N；平均滑動摩擦力約 ${scenario.kineticFrictionMeanN.toFixed(2)} N。</p></details>` : "";
    ...
    function render() {
      ...
      if (!state) { showPanel(null); renderResult(); }
        ...
        if (state.phase === "experiment" && state.trial && !experimentQuality) setText("experimentStatus", "記錄已保存，可以前往 Part C。");
    ...
    function applyAttempt(attempt) {
      ...
      presentation = "load-error"; if (typeof document !== "undefined") { setText("technicalTitle", "活動暫時無法使用"); setText("technicalMessage", "暫時無法讀取這次活動，未能確認本次作答；請稍後重試。"); } render(); return false;
    }
    ...
    function validationMessage(action) {
      ...
      if (action === "save-prediction") return "請先在舞台上由物體中央畫出摩擦力箭頭；不畫代表摩擦力為零。然後選擇摩擦力類型及運動結果。";
    ...
    function wireEvents() {
      ...
      document.addEventListener("click", (event) => {
        ...
        try {
          ...
          else if (action === "draw-friction") { balanceInteractionMode = "static"; balanceDrawMode = "friction"; balanceFrictionCleared = false; announce("請由物體中央拖出摩擦力箭頭；不畫箭頭代表沒有摩擦力"); }
          else if (action === "clear-friction") { balanceInteractionMode = "static"; balanceDrawings.friction = null; balanceFrictionCleared = true; balanceDrawMode = "friction"; announce("已清除摩擦力箭頭，代表沒有摩擦力"); }
          ...
          else if (action === "request-redo-experiment") requestRedoExperiment();
          ...
          else if (action === "edit-experiment") requestRedoExperiment();
          ...
          else if (action === "retry-finish") { const outcome = typeof SimScorm !== "undefined" ? SimScorm.retryFinish?.() : null; if (outcome?.ok) { presentation = "submitted-success"; render(); } else if (outcome?.committed) { setText("submitStatus", "答案和分數已安全保存，活動亦已鎖定；請稍後重試完成提交。"); } }
    ...
    function submit() {
      ...
      const callbacks = {
        ...
        committed: () => { latestResult = result; presentation = "submitted-committed"; state = reviewState; render(); focusReviewSurface(); setText("submitStatus", "答案和分數已保存，但系統尚未完成提交；請稍後重試。"); },
