...
async function completeLearnerPath(cdp, baseUrl, launchPath, label, keyboard = false) {
  ...
  const designInitial = await evaluate(cdp, "(() => { const zero=[...document.querySelectorAll('#stageSvg text')].find((node)=>node.textContent==='0.0 cm'),ceiling=[...document.querySelectorAll('#stageSvg line')].find((node)=>node.getAttribute('x1')==='96'&&node.getAttribute('x2')==='700'&&node.getAttribute('y1')==='62'); return {heading:document.querySelector('#designPanel h2')?.textContent||'',calculationState:document.getElementById('designCalculation')?.dataset.state||'',stageText:[...document.querySelectorAll('#stageSvg text')].map((node)=>node.textContent),zeroY:Number(zero?.getAttribute('y')),ceilingY:Number(ceiling?.getAttribute('y1'))}; })()");
  ...
  const designCalculation = await evaluate(cdp, "({calculation:document.getElementById('designCalculation')?.textContent||'',summary:document.getElementById('designSummary')?.textContent||'',stageText:[...document.querySelectorAll('#stageSvg text')].map((node)=>node.textContent),kA:document.getElementById('designK_A')?.textContent||'',loadWidth:Number(document.querySelector('#stageSvg [data-role=design-load]')?.getAttribute('width'))})");
  ...
  const largerDesignLoad = await evaluate(cdp, "Number(document.querySelector('#stageSvg [data-role=design-load]')?.getAttribute('width'))");
  assert.ok(largerDesignLoad > designCalculation.loadWidth, `${label}: increasing the load makes the single hanging block wider`);
