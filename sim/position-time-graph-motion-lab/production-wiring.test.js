...
assert.match(indexSource, /<div id="graphTouchPreviewHost" class="diagram-magnifier-host graph-magnifier-host" aria-hidden="true" hidden><\/div>/, "graph touch magnifier uses an overlay outside the source graph SVG");
...
assert.match(source, /R\.hitRadius\(800, roadRenderedWidth\(\), 26, 52\)/, "road car and velocity targets request real CTM-aware 52 CSS px geometry");
...
assert.match(stylesSource, /\.road-drag-hit\s*\{[^}]*stroke:\s*none[^}]*pointer-events:\s*all[^}]*touch-action:\s*none/s, "road targets use explicit SVG geometry without legacy non-scaling transparent strokes");
assert.match(source, /function nearestRoadDragTarget\(event\)[\s\S]*distance <= 28[\s\S]*sort\(\(a, b\) => a\.distance - b\.distance\)/, "overlapping road targets resolve by nearest semantic visual focus");
assert.match(stylesSource, /\.diagram-magnifier\s*\{[^}]*width:\s*100%[^}]*height:\s*100%[^}]*pointer-events:\s*none/s, "real diagram magnifiers cannot intercept dragging or outer-page gestures");
assert.match(stylesSource, /\.diagram-magnifier-host\s*\{[^}]*position:\s*absolute[^}]*width:\s*min\(52vw, 11\.5rem\)[^}]*pointer-events:\s*none/s, "magnifier overlays have readable CSS dimensions without changing diagram layout");
assert.match(source, /magnifierSource\(dom\.graphLayer\)/, "graph magnifier clones the current rendered x-t graph rather than generating a summary graphic");
assert.match(source, /magnifierSource\(dom\.roadLayer\)/, "road magnifier clones the current rendered road rather than generating a summary graphic");
assert.match(source, /graphMagnifierViewBox\(name, position\)/, "magnifier crop follows the authoritative dragged endpoint position");
assert.doesNotMatch(source, /graph-touch-preview-(?:title|value|mini|point)|data-preview-(?:point|value)/, "summary preview titles, values, mini axes, and synthetic points are removed");
assert.match(stylesSource, /\.road-block,\s*\.graph-block\s*\{[^}]*position:\s*relative/s, "preview overlays are positioned against their source diagram blocks rather than the whole stage");
assert.match(stylesSource, /\.lab-stage\s*\{[^}]*position:\s*relative/s, "road preview is positioned against the full visible stage rather than the clipped road row");
...
function velocityHitState() {
  ...
  const carContainsVelocityCenter = Math.hypot(x - numericAttribute(car, "cx"), y - numericAttribute(car, "cy")) <= numericAttribute(car, "r") + numericAttribute(velocity, "r");
...
const explorePreviewHit = velocityHitState();
document.getElementById("roadSvg").dispatch("pointerdown", { target: explorePreviewHit.velocity, pointerId: 6, pointerType: "touch", isPrimary: true, clientX: explorePreviewHit.x, clientY: explorePreviewHit.y });
const roadMagnifierMarkup = document.getElementById("roadTouchPreviewHost").innerHTML;
assert.ok(roadMagnifierMarkup.includes('class="diagram-magnifier road-magnifier"'), "exploration touch drag immediately shows a real road magnifier");
assert.ok(roadMagnifierMarkup.includes('class="road-track"'), "road magnifier contains the source road track");
assert.ok(roadMagnifierMarkup.includes('class="car-body"'), "road magnifier contains the source car geometry");
assert.equal(roadMagnifierMarkup.includes("data-drag="), false, "road magnifier strips cloned drag targets");
assert.equal(roadMagnifierMarkup.includes("tabindex="), false, "road magnifier strips cloned focus targets");
document.getElementById("roadSvg").dispatch("pointercancel", { pointerId: 6, pointerType: "touch" });
assert.equal(document.getElementById("roadTouchPreviewHost").children.length, 0, "road magnifier clears on pointer cancellation");
const roadJitterHit = velocityHitState();
const roadJitterValue = Number(roadJitterHit.velocity.getAttribute("aria-valuenow"));
const roadJitterSaves = savedDrafts.length;
document.getElementById("roadSvg").dispatch("pointerdown", { target: roadJitterHit.velocity, pointerId: 60, pointerType: "touch", isPrimary: true, clientX: roadJitterHit.x, clientY: roadJitterHit.y });
document.getElementById("roadSvg").dispatch("pointermove", { pointerId: 60, pointerType: "touch", clientX: roadJitterHit.x + 2, clientY: roadJitterHit.y });
document.getElementById("roadSvg").dispatch("pointerup", { pointerId: 60, pointerType: "touch", clientX: roadJitterHit.x + 2, clientY: roadJitterHit.y });
assert.equal(Number(velocityHitState().velocity.getAttribute("aria-valuenow")), roadJitterValue, "sub-threshold road jitter leaves velocity unchanged");
assert.equal(savedDrafts.length, roadJitterSaves, "sub-threshold road jitter creates no draft save");
assert.equal(document.getElementById("roadTouchPreviewHost").children.length, 0, "sub-threshold road jitter still clears preview on release");
const roadSnapHit = velocityHitState();
document.getElementById("roadSvg").dispatch("pointerdown", { target: roadSnapHit.velocity, pointerId: 61, pointerType: "touch", isPrimary: true, clientX: roadSnapHit.x, clientY: roadSnapHit.y });
document.getElementById("roadSvg").dispatch("pointermove", { pointerId: 61, pointerType: "touch", clientX: roadSnapHit.x + 6, clientY: roadSnapHit.y });
document.getElementById("roadSvg").dispatch("pointerup", { pointerId: 61, pointerType: "touch", clientX: roadSnapHit.x + 6, clientY: roadSnapHit.y });
assert.equal(Number(velocityHitState().velocity.getAttribute("aria-valuenow")), roadJitterValue, "activated road move inside the same snap cell leaves velocity unchanged");
assert.equal(savedDrafts.length, roadJitterSaves, "activated road move without a semantic change creates no draft save");
...
for (const initialVelocity of [0, 0.5, -0.5]) {
  ...
  const hit = velocityHitState();
    ...
    document.getElementById("roadSvg").dispatch("pointermove", { pointerId: 7, clientX: hit.x + 24, clientY: hit.y });
...
const missionOneJitter = one('[data-drag="initial:x0"]');
const missionOneJitterSaves = savedDrafts.length;
document.getElementById("graphSvg").dispatch("pointerdown", { target: missionOneJitter, pointerId: 62, pointerType: "touch", isPrimary: true, clientX: 80, clientY: 225 });
document.getElementById("graphSvg").dispatch("pointermove", { pointerId: 62, pointerType: "touch", clientX: 80, clientY: 223 });
document.getElementById("graphSvg").dispatch("pointerup", { pointerId: 62, pointerType: "touch", clientX: 80, clientY: 223 });
assert.equal(Object.prototype.hasOwnProperty.call(draftProvider().answer.a.ans.m1, "x0"), false, "sub-threshold initial-point jitter keeps unanswered x0 absent");
assert.equal(savedDrafts.length, missionOneJitterSaves, "sub-threshold initial-point jitter creates no draft save");
assert.equal(document.getElementById("graphTouchPreviewHost").children.length, 0, "sub-threshold initial-point jitter clears preview on release");
...
assert.equal(document.querySelectorAll('[data-drag="initial:x0"]').length, 0, "mission 1 initial point becomes read-only away from zero seconds");
assert.ok(document.getElementById("graphLayer").innerHTML.includes(`class="current-dot" cx="${80 + 0.5 / 6 * 680}`), "mission 1 current dot continues following playback after the initial handle locks");
...
document.getElementById("timeSlider").value = "0";
document.getElementById("timeSlider").dispatch("input");
const missionOneInitial = one('[data-drag="initial:x0"]');
assert.ok(Number(missionOneInitial.getAttribute("width")) >= 52 && Number(missionOneInitial.getAttribute("height")) >= 52, "mission 1 initial graph point has a real 52px hit area");
const missionOneSaves = savedDrafts.length;
const missionOneVelocity = savedDrafts.at(-1).answer.a.ans.m1.v;
document.getElementById("graphSvg").dispatch("pointerdown", { target: missionOneInitial, pointerId: 30, pointerType: "touch", isPrimary: true, clientX: 80, clientY: 225 });
assert.ok(document.getElementById("graphTouchPreviewHost").innerHTML.includes('data-graph-point="initial"'), "mission 1 initial-point touch drag shows the real source graph in its magnifier");
document.getElementById("graphSvg").dispatch("pointermove", { pointerId: 30, pointerType: "touch", clientX: 80, clientY: 192 });
document.getElementById("graphSvg").dispatch("pointerup", { pointerId: 30, pointerType: "touch", clientX: 80, clientY: 192 });
assert.notEqual(savedDrafts.at(-1).answer.a.ans.m1.x0, undefined, "mission 1 graph drag writes the existing x0 answer field");
assert.equal(savedDrafts.at(-1).answer.a.ans.m1.v, missionOneVelocity, "mission 1 graph drag leaves velocity unchanged");
assert.equal(savedDrafts.length, missionOneSaves + 1, "mission 1 graph drag saves exactly once on release");
assert.equal(document.getElementById("graphTouchPreviewHost").children.length, 0, "mission 1 graph magnifier clears on release");
const missionOneX0Range = quantity("x0");
missionOneX0Range.focus();
missionOneX0Range.value = "0";
missionOneX0Range.dispatch("input");
missionOneX0Range.dispatch("change");
assert.equal(document.activeElement?.dataset.quantity, "x0", "range adjustment keeps keyboard focus on the corresponding x0 control");
...
assert.ok(document.getElementById("graphTouchPreviewHost").innerHTML.includes('viewBox="0 135 280 180"'), "secondary pointer cannot replace the active P0 magnifier crop with P6");
...
const magnifierMarkup = document.getElementById("graphTouchPreviewHost").innerHTML;
assert.ok(magnifierMarkup.includes('class="diagram-magnifier graph-magnifier"'), "touch graph drag immediately shows a real SVG magnifier");
assert.ok(magnifierMarkup.includes('class="plot-grid"'), "magnifier contains the source graph grid");
assert.ok(magnifierMarkup.includes('class="motion-line student-line"'), "magnifier contains the source answer line");
assert.ok(magnifierMarkup.includes('data-graph-point="P0"'), "magnifier contains the source P0 graph geometry");
assert.equal(magnifierMarkup.includes('data-drag='), false, "magnifier removes cloned drag targets");
assert.equal(magnifierMarkup.includes('tabindex='), false, "magnifier removes cloned keyboard focus targets");
assert.equal(magnifierMarkup.includes('graph-touch-preview-title'), false, "magnifier adds no title or explanatory text");
...
assert.equal(document.getElementById("graphTouchPreviewHost").children.length, 0, "pointer cancel hides the graph magnifier");
...
const missionFourInitial = one('[data-drag="initial:x0"]');
const missionFourSaves = savedDrafts.length;
document.getElementById("graphSvg").dispatch("pointerdown", { target: missionFourInitial, pointerId: 31, pointerType: "touch", isPrimary: true, clientX: 80, clientY: 225 });
assert.ok(document.getElementById("graphTouchPreviewHost").innerHTML.includes('data-graph-point="initial"'), "mission 4 initial-point touch drag shows the real graph magnifier");
document.getElementById("graphSvg").dispatch("pointermove", { pointerId: 31, pointerType: "touch", clientX: 80, clientY: 242 });
document.getElementById("graphSvg").dispatch("pointerup", { pointerId: 31, pointerType: "touch", clientX: 80, clientY: 242 });
assert.equal(savedDrafts.length, missionFourSaves + 1, "mission 4 initial-point drag saves exactly once");
quantity("x0").value = "0";
quantity("x0").dispatch("input");
quantity("x0").dispatch("change");
one('[data-set-quantity="velocity"]').click();
...
const missionFiveRoad = document.getElementById("roadLayer").innerHTML;
assert.ok(missionFiveRoad.includes('data-road-car="A"') && missionFiveRoad.includes('data-road-car="B"'), "mission 5 renders both labelled cars in separate road groups");
assert.ok(missionFiveRoad.includes('transform="translate(') && missionFiveRoad.includes(' 76)') && missionFiveRoad.includes(' 29)'), "mission 5 assigns A and B to distinct visible vertical lanes");
assert.ok(missionFiveRoad.includes('data-drag="car:B"') && missionFiveRoad.includes('data-drag="velocity:B"'), "mission 5 keeps both B car and velocity directly draggable");
assert.equal(missionFiveRoad.includes('data-drag="car:A"'), false, "mission 5 keeps authoritative A car read-only");
const missionFiveB = one('[data-drag="car:B"]');
document.getElementById("roadSvg").dispatch("pointerdown", { target: missionFiveB, pointerId: 32, pointerType: "touch", isPrimary: true, clientX: Number(missionFiveB.dataset.focusX), clientY: Number(missionFiveB.dataset.focusY) });
assert.ok(document.getElementById("roadTouchPreviewHost").innerHTML.includes('data-road-car="B"'), "mission 5 B drag shows the real two-car road in its magnifier");
document.getElementById("roadSvg").dispatch("pointercancel", { pointerId: 32, pointerType: "touch" });
