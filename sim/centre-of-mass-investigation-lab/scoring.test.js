...
function complete(seed=3){let s=P.initial(seed),p=G.generate(seed);s=P.release(s,p.part1.xCm);s=P.markPart1(s,p.part1.xCm);s=P.switchPart(s,2);for(const key of ["h1","h2"]){s=P.settleHole(s,key);s=P.traceVertical(s);}s=P.markPart2(s,p.part2.centre);s=P.switchPart(s,3);s=P.setView(s,{yaw10:650,pitch10:-180});s=P.setView(s,{yaw10:1100,pitch10:220});s=P.selectPart3(s,p.part3.correctKey);return P.enterCheck(s);}
