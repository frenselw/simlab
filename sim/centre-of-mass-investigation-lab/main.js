(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function routeStartup(outcome, attempt) {
      markDraft=null;selectedHole=null;clearGesture(); const view = UiPolicy.startupView(outcome); locked = view.locked;
      if (outcome === "editable") {
    ...
    function syncPart2Targets() {
      ...
      const mark = dom.direct.querySelector('[data-direct-target="mark2"]'); if (mark) { const point = markDraft?.world || (state.part2.mark ? localToSvg(state.part2.mark) : {x:650,y:390}); setDirectSvgPosition(mark,point); }
    ...
    function renderSolid(canvas, compact = false) {
      ...
      const projected = problem.part3.candidates.map((item) => ({ item, p: Model.project(item.position, state.part3.view, scale) })).sort((a,b) => a.p.depth-b.p.depth); for (const {item,p} of projected) { ctx.globalAlpha = .95; ctx.fillStyle = item.key === state.part3.selectedCandidateKey ? "#dc2626" : "#2563eb"; ctx.beginPath(); ctx.arc(cx+p.x,cy+p.y,compact?5:8,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
