(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function resetPlateAfterLine(next) { const from={...platePose},to={x:350,y:235,angle:0},started=null,duration=matchMedia("(prefers-reduced-motion: reduce)").matches?160:430;const animate=(time)=>{if(started===null)started=time;const t=Math.min(1,(time-started)/duration),e=1-Math.pow(1-t,3);platePose={x:from.x+(to.x-from.x)*e,y:from.y+(to.y-from.y)*e,angle:from.angle+(to.angle-from.angle)*e};renderPart2(true);syncPart2Targets();if(t<1)swingFrame=requestAnimationFrame(animate);else{swingFrame=0;update(next,"已記錄鉛垂線；平板已取下，可改用另一個小孔。");}};swingFrame=requestAnimationFrame(animate); }
    ...
    function ensurePart3Renderer(){if(part3Renderer||part3Attempted)return;part3Attempted=true;part3Mode="loading";dom.rendererBadge.textContent="載入空間視圖…";import("./part3-renderer.js").then(()=>{if(globalThis.__CENTRE_MASS_FORCE_FALLBACK)throw new Error("forced fallback");part3Renderer=globalThis.CentreMassPart3Renderer.create(dom.canvas,problem.part3,(event)=>{part3Mode=event==="restored"?"three":"fallback";if(state?.phase==="part3")render();});part3Mode="three";if(state?.phase==="part3")render();}).catch(()=>{part3Mode="fallback";part3Renderer=null;if(state?.phase==="part3")render();});}
    ...
    function rollbackInteraction(){const baseline=gesture?.baselineState,support=gesture?.baselineSupport,pose=gesture?.baselinePose;clearGesture();if(baseline){state=baseline;problem=Generator.generate(state.seed,state.generatorVersion);supportX=support;platePose=pose;render();}}
