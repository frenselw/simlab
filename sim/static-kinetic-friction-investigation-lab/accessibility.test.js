...
for (const target of ["balance-origin", "prediction-friction", "breakaway-marker", "static-start", "static-end"]) assert.match(html, new RegExp(`data-drag-target="${target}"`));
...
assert.match(html, /id="experimentForceSlider"[^>]*type="range"/);
...
assert.match(css, /\.experiment-force-slider[^}]*touch-action:\s*none/);
...
assert.match(main, /experimentForceSlider/);
...
assert.doesNotMatch(main, /experimentOrigin|experiment-pull/);
