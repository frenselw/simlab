(function (root, factory) {
  ...
  const NAIL = Object.freeze({ x: 350, y: 140 });
  ...
  function boot(options = {}) {
    ...
    const dom = { app: $("app"), stage: $("stage"), svg: $("svgStage"), canvas: $("solidCanvas"), fallbackCanvas: $("fallbackCanvas"), direct: $("directLayer"), preview: $("preview"), panel: $("controlPanel"),
    ...
    function bind() {
      ...
      dom.stage.addEventListener("touchstart", (event) => {
        if (state?.phase === "part2" && (event.target?.closest?.("[data-direct-target]") || event.target?.matches?.(".student-line,.student-line-hit,.plumb,.plumb-hit,.plumb-bob,[data-scene=draw-ghost]"))) event.preventDefault();
      }, { passive: false });
      dom.stage.addEventListener("pointerdown", (event) => {
        if (state?.phase === "part2" && event.target?.matches?.(".student-line,.student-line-hit,.plumb,.plumb-hit,.plumb-bob")) {
          event.preventDefault();
          const plate = dom.direct.querySelector('[data-direct-target="plate"]');
          if (plate && pointInMaterial(svgToLocal(clientPoint(event)))) beginPlateDrag(event, plate);
          else announce("請按住平板拖動，或從懸掛孔開始畫線。");
          return;
        }
        if (event.pointerType === "touch" && !gesture && !event.defaultPrevented && UiPolicy.pointerOwner(event.target) === "host") hostSwipe = { id: event.pointerId, y: event.clientY };
      });
    ...
    function applySwingPose(hole) { platePose.angle = swingRuntime.angle; alignHoleToNail(hole); renderPart2(true); syncPart2Targets(); }
    ...
    function render() {
      if (!state || locked) return; hideAll(); const isPart3=state.phase==="part3";dom.svg.style.display=isPart3?"none":"block";dom.canvas.style.display=isPart3&&part3Mode==="three"?"block":"none";dom.fallbackCanvas.style.display=isPart3&&part3Mode!=="three"?"block":"none";dom.direct.replaceChildren(); dom.preview.classList.add("is-hidden");
      ...
      if(/^part[123]$/.test(state.phase))lastPart=Number(state.phase.slice(-1));dom.tabs.forEach((tab,index)=>{const active=state.phase===`part${index+1}`,roving=active||(!state.phase.startsWith("part")&&index===lastPart-1);tab.disabled=false;tab.setAttribute("aria-selected",String(active));tab.tabIndex=roving?0:-1;tab.dataset.complete=String(complete[index]);tab.setAttribute("aria-label",`${tab.textContent}，${complete[index]?"已記錄":"待完成"}`);});dom.checkTab.disabled=false;dom.checkTab.setAttribute("aria-current",state.phase==="check"?"step":"false");
    ...
    function beginPlateDrag(event,target,preferredKey=null) { if(gesture||locked||event.button>0||swingRuntime||!preferredKey&&!pointInMaterial(svgToLocal(clientPoint(event)))||!prepareTakeDown())return;beginDrag(event,target,preferredKey?`hole-${preferredKey}`:"plate",(point,previous)=>{translatePlate(point.x-previous.x,point.y-previous.y);updateSnapCandidate(preferredKey);},()=>finishPlateDrop(preferredKey)); }
    function syncPart2Targets() {
      ...
      const drawStartMaterial = dom.direct.querySelector("[data-draw-start-material]"); if (drawStartMaterial) drawStartMaterial.setAttribute("d",platePath());
      const drawStart = dom.direct.querySelector('[data-direct-target="draw-start"]'); if (drawStart && state.part2.activeHoleKey) { const hole=problem.part2.holes.find((item)=>item.key===state.part2.activeHoleKey);if(hole){const point=localToSvg(hole),radii=drawStartRadii();drawStart.setAttribute("cx",point.x);drawStart.setAttribute("cy",point.y);drawStart.setAttribute("rx",radii.x);drawStart.setAttribute("ry",radii.y);} }
    }
    ...
    function renderPart2(visualsOnly = false) {
      ...
      if(state.part2.activeHoleKey){dom.svg.append(svgEl("line",{x1:NAIL.x,y1:NAIL.y+10,x2:NAIL.x,y2:420,class:"plumb"}));if(state.phase==="part2")dom.svg.append(svgEl("line",{x1:NAIL.x,y1:NAIL.y+10,x2:NAIL.x,y2:420,class:"plumb-hit"}));}
      for (const line of state.part2.lines) { const a = localToSvg({ x: line.a[0], y: line.a[1] }), b = localToSvg({ x: line.b[0], y: line.b[1] }); dom.svg.append(svgEl("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, class: "student-line" })); if(state.phase==="part2")dom.svg.append(svgEl("line",{x1:a.x,y1:a.y,x2:b.x,y2:b.y,class:"student-line-hit"})); }
      ...
      if(drawGhost)dom.svg.append(svgEl("line",{x1:drawGhost.a.x,y1:drawGhost.a.y,x2:drawGhost.b.x,y2:drawGhost.b.y,stroke:drawGhost.snapped?"#b91c1c":"#b45309","stroke-width":6,"stroke-linecap":"round","stroke-dasharray":drawGhost.snapped?"none":"8 5","data-scene":"draw-ghost"}));
      if(state.part2.activeHoleKey)dom.svg.append(svgEl("path",{d:`M${NAIL.x} 418l-12 24h24Z`,fill:"#374b57",stroke:"#1f3039","stroke-width":3,class:"plumb-bob"}));
    ...
    function materialTarget(label,kind,className="",focusable=true) { const layer=svgEl("svg",{viewBox:`0 0 ${STAGE_WIDTH} ${STAGE_HEIGHT}`,class:`material-target-layer ${className}`.trim()}),attrs={d:platePath(),fill:"rgba(0,0,0,.001)","fill-rule":"evenodd","pointer-events":"fill"};if(focusable)Object.assign(attrs,{tabindex:"0",role:"button","aria-label":label});else attrs["aria-hidden"]="true";const path=svgEl("path",attrs);path.dataset.directTarget=kind;layer.append(path);dom.direct.append(layer);return path; }
    function drawStartRadii() { const rect=dom.stage.getBoundingClientRect();return{x:48*STAGE_WIDTH/Math.max(1,rect.width),y:48*STAGE_HEIGHT/Math.max(1,rect.height)}; }
    function clippedDrawStartTarget(label,point) { const layer=svgEl("svg",{viewBox:`0 0 ${STAGE_WIDTH} ${STAGE_HEIGHT}`,class:"material-target-layer draw-start-layer"}),defs=svgEl("defs"),clip=svgEl("clipPath",{id:"draw-start-material-clip",clipPathUnits:"userSpaceOnUse"}),material=svgEl("path",{d:platePath(),"fill-rule":"evenodd","clip-rule":"evenodd","data-draw-start-material":""}),radii=drawStartRadii(),target=svgEl("ellipse",{cx:point.x,cy:point.y,rx:radii.x,ry:radii.y,fill:"rgba(0,0,0,.001)","clip-path":"url(#draw-start-material-clip)","pointer-events":"fill",tabindex:"0",role:"button","aria-label":label});target.dataset.directTarget="draw-start";clip.append(material);defs.append(clip);layer.append(defs,target);dom.direct.append(layer);return target; }
