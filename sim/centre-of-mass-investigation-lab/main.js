(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function render() {
      ...
      dom.tabs.forEach((tab,index)=>{const active=state.phase===`part${index+1}`;tab.setAttribute("aria-selected",String(active));tab.tabIndex=active?0:-1;tab.dataset.complete=String(complete[index]);});dom.checkTab.disabled=false;dom.checkTab.setAttribute("aria-current",state.phase==="check"?"step":"false");
    ...
    function renderSolid(canvas, compact = false) {
      ...
      const projected = problem.part3.candidates.map((item) => ({ item, p: Model.project(item.position, state.part3.view, scale) })).sort((a,b) => a.p.depth-b.p.depth); for (const {item,p} of projected) { const radius=compact?4.5:7;if(item.key===state.part3.selectedCandidateKey){ctx.strokeStyle="#1e3a8a";ctx.lineWidth=compact?3:4;ctx.beginPath();ctx.arc(cx+p.x,cy+p.y,radius+3,0,Math.PI*2);ctx.stroke();ctx.strokeStyle="#fff";ctx.lineWidth=compact?1.5:2;ctx.beginPath();ctx.arc(cx+p.x,cy+p.y,radius+2,0,Math.PI*2);ctx.stroke();}ctx.globalAlpha = 1; ctx.fillStyle = Generator.CANDIDATE_COLORS[item.key]; ctx.beginPath(); ctx.arc(cx+p.x,cy+p.y,radius,0,Math.PI*2); ctx.fill(); } ctx.globalAlpha=1;
