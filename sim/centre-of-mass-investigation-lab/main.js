(function (root, factory) {
  ...
  function boot(options = {}) {
    ...
    function syncPart2Targets() {
      ...
      const draw = dom.direct.querySelector('[data-direct-target="draw"]'); if (draw) draw.style.clipPath = `polygon(${directSvgPolygon(problem.part2.polygon.map((point)=>localToSvg({x:point[0],y:point[1]})))})`;
    }
    ...
    function renderSolid(canvas, compact = false) {
      ...
      const projected = problem.part3.candidates.map((item) => ({ item, p: Model.project(item.position, state.part3.view, scale) })).sort((a,b) => a.p.depth-b.p.depth); for (const {item,p} of projected) { ctx.globalAlpha = .95; ctx.fillStyle = Generator.CANDIDATE_COLORS[item.key]; ctx.beginPath(); ctx.arc(cx+p.x,cy+p.y,compact?4.5:7,0,Math.PI*2); ctx.fill(); if(item.key===state.part3.selectedCandidateKey){ctx.strokeStyle="#fff";ctx.lineWidth=compact?1.5:2;ctx.beginPath();ctx.arc(cx+p.x,cy+p.y,(compact?4.5:7)+3,0,Math.PI*2);ctx.stroke();} } ctx.globalAlpha=1;
    ...
    function syncPart3Targets(projected) {
      const rect=dom.stage.getBoundingClientRect(),arranged=layoutCandidateTargets(projected,Math.max(1,rect.width),Math.max(1,rect.height));
      lastProjection=arranged;
      const leaders=dom.direct.querySelector(".candidate-leaders");
      if(leaders){leaders.replaceChildren();for(const item of arranged)if(item.displacementCss>16)leaders.append(svgEl("line",{x1:item.anchorX,y1:item.anchorY,x2:item.x,y2:item.y}),svgEl("circle",{cx:item.anchorX,cy:item.anchorY,r:3}));}
      for(const item of arranged){const button=dom.direct.querySelector(`[data-direct-target="candidate-${item.key}"]`);if(!button)continue;button.style.left=`${item.x/7}%`;button.style.top=`${item.y/4.6}%`;button.dataset.anchorX=String(item.anchorX);button.dataset.anchorY=String(item.anchorY);button.setAttribute("aria-label",`候選點 ${item.key}${item.depth<0?"，在後方":"，在前方"}`);}
      return arranged;
    }
    function renderPart3() {
      dom.svg.replaceChildren();ensurePart3Renderer();dom.canvas.style.display=part3Mode==="three"?"block":"none";dom.fallbackCanvas.style.display=part3Mode==="three"?"none":"block";dom.rendererBadge.textContent=part3Mode==="three"?(part3Renderer?.label||"Three.js 0.185.1"):part3Mode==="loading"?"載入空間視圖…":"Canvas 相容模式";let projected;try{projected=part3Mode==="three"&&part3Renderer?part3Renderer.render(state.part3.view,state.part3.selectedCandidateKey):renderSolid(dom.fallbackCanvas);}catch{part3Mode="fallback";part3Renderer?.dispose?.();part3Renderer=null;dom.canvas.style.display="none";dom.fallbackCanvas.style.display="block";projected=renderSolid(dom.fallbackCanvas);}dom.direct.replaceChildren();dom.direct.append(svgEl("svg",{viewBox:"0 0 700 460",class:"candidate-leaders","aria-hidden":"true"})); const orbit = directButton(50,50,"拖動立體改變觀察方向", "orbit"); orbit.classList.add("orbit-target"); orbit.addEventListener("keydown", (event) => { if (!event.key.startsWith("Arrow")) return; const step = event.shiftKey ? 150 : 50; event.preventDefault(); update(Persistence.setView(state,{yaw10:state.part3.view.yaw10+(event.key==="ArrowLeft"?-step:event.key==="ArrowRight"?step:0),pitch10:state.part3.view.pitch10+(event.key==="ArrowUp"?-step:event.key==="ArrowDown"?step:0)}),"已記錄新的觀察方向。" ); }); orbit.addEventListener("pointerdown", (event) => beginOrbit(event, orbit));
      ...
      for (const item of projected) { const button = directButton(item.x/7,item.y/4.6,`候選點 ${item.key}${item.depth<0?"，在後方":"，在前方"}`,`candidate-${item.key}`,item.key);button.classList.add("candidate-target");button.dataset.candidateKey=item.key;button.style.setProperty("--candidate-color",Generator.CANDIDATE_COLORS[item.key]);button.classList.toggle("is-selected",state.part3.selectedCandidateKey===item.key);button.classList.toggle("is-observation-locked",!observationReady);button.setAttribute("aria-pressed",String(state.part3.selectedCandidateKey===item.key));button.setAttribute("aria-disabled",String(!observationReady));button.addEventListener("click",()=>{if(!observationReady){announce("請先拖動立體，從不同方向觀察；完成一次旋轉後即可按候選點。");return;}update(Persistence.selectPart3(state,item.key),state.part3.observations.length<2?`已暫選候選點 ${item.key}；如要完成觀察證據，請再轉動一次。`:`已選擇候選點 ${item.key}。`);});button.addEventListener("pointerdown",(event)=>showTapPreview(event,button,`candidate-${item.key}`)); }
      syncPart3Targets(projected);
      dom.radios.replaceChildren(dom.radios.querySelector("legend") || Object.assign(document.createElement("legend"),{textContent:"選擇重心候選點"}), ...Generator.LABELS.map((key)=>{const label=document.createElement("label"),input=document.createElement("input"),swatch=document.createElement("span");input.type="radio";input.name="candidate";input.value=key;input.disabled=!observationReady;input.checked=state.part3.selectedCandidateKey===key;swatch.className="candidate-swatch";swatch.style.setProperty("--candidate-color",Generator.CANDIDATE_COLORS[key]);swatch.setAttribute("aria-hidden","true");input.addEventListener("change",()=>{if(!observationReady)return;update(Persistence.selectPart3(state,key),state.part3.observations.length<2?`已暫選候選點 ${key}；如要完成觀察證據，請再轉動一次。`:`已選擇候選點 ${key}。`);});label.append(input,swatch,` ${key} `);return label;}));
