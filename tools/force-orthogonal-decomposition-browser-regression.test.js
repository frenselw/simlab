...
assert.doesNotMatch(html, /keyboard-section|鍵盤替代操作|Tab 聚焦透明控制點/);
...
assert.match(styles, /\.force-shell\s*\{[\s\S]*grid-template-rows:\s*minmax\(0,\s*min\(calc\(44vh \+ 4rem\),\s*calc\(100% - 6rem\)\)\)\s+minmax\(0,\s*1fr\)/s);
...
assert.match(main, /formulaDone \? formulaNextLabel\(\) : "完成分力表達式"/);
assert.match(main, /第 5 步「分力表達式」已完成/);
assert.match(main, /switchQuestion\(nextIndex, false, `第 5 步/);
