...
let sameValueWithPredictionDraft = P.transitions.setPhase(state, "predict");
const sameValuePredictionDraft = { ...state.predictions[0], frictionType: state.predictions[0].frictionType === "kinetic" ? "static" : "kinetic", committed: false };
sameValueWithPredictionDraft = P.transitions.setPrediction(sameValueWithPredictionDraft, 0, sameValuePredictionDraft);
sameValueWithPredictionDraft = P.transitions.setPhase(sameValueWithPredictionDraft, "analysis");
const sameValueWithPredictionDraftSave = P.transitions.setAnalysisMarkers(sameValueWithPredictionDraft, Object.fromEntries(P.ANALYSIS_KEYS.map((key) => [key, { index: canonicalAnalysis[key].index, committed: true }])));
assert.deepEqual(sameValueWithPredictionDraftSave.predictions, canonicalPredictions, "same-value C save preserves canonical D answers when a D draft exists");
assert.deepEqual(sameValueWithPredictionDraftSave.working.predictionDraft[0], sameValuePredictionDraft, "same-value C save preserves the separate D working draft");
...
let draftOnlyPredictionWork = P.transitions.setPrediction(requiredOnly, 0, { id: firstPredictionSpec.id, scenarioId: firstPredictionSpec.scenarioId, frictionType: firstPredictionSpec.frictionType, direction: null, magnitudeCN: null, motionOutcome: null, committed: false });
draftOnlyPredictionWork = P.transitions.setPhase(draftOnlyPredictionWork, "analysis");
const changedValueWithDraftOnly = P.transitions.setAnalysisMarkers(draftOnlyPredictionWork, Object.fromEntries(P.ANALYSIS_KEYS.map((key) => [key, { index: key === "kineticFriction" ? changedKineticIndex : canonicalAnalysis[key].index, committed: true }])));
assert.deepEqual(changedValueWithDraftOnly.predictions, [null, null, null, null], "changed C save invalidates D when only a working D draft exists");
assert.deepEqual(changedValueWithDraftOnly.working.predictionDraft, [null, null, null, null], "changed C save clears the separate D working draft as well");
...
const preWorkingV51Wire = P.clone(P.encodeDraft(oldMeasurementState)); preWorkingV51Wire.P[0][6] = 0; delete preWorkingV51Wire.k.m; delete preWorkingV51Wire.k.n;
const migratedV51WorkingPrediction = P.decodeSnapshot({ version: 1, activity: ACTIVITY, kind: "draft", answer: preWorkingV51Wire }, scenario, "draft");
assert.equal(migratedV51WorkingPrediction.predictions[0], null, "a pre-working s6 draft does not keep an uncommitted prediction in canonical P");
assert.deepEqual(migratedV51WorkingPrediction.working.predictionDraft[0], { id: oldMeasurementSpec.id, scenarioId: oldMeasurementSpec.scenarioId, frictionType: oldMeasurementSpec.frictionType, direction: oldMeasurementSpec.direction, magnitudeCN: oldMeasurementSpec.magnitudeCN, motionOutcome: oldMeasurementSpec.motionOutcome, committed: false }, "a pre-working s6 draft moves an uncommitted P entry into working predictionDraft");
