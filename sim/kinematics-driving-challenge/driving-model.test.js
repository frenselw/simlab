...
assert.equal(Model.CONTROL_LABELS.length, 3);
assert.equal(Model.accelerationFor(8, 0, 1), Model.PEDAL_ACCELERATION);
assert.equal(Model.accelerationFor(8, 0, 0), 0);
assert.equal(Model.accelerationFor(8, 0, 2), -Model.PEDAL_ACCELERATION);
...
assert.equal(Model.accelerationFor(0, 0, 2), 0, "a stopped car does not reverse under braking");
...
assert.equal(Model.replay(level, [3]), null);
assert.equal(Model.isTerminalRun(level, [1]), false);
const queued = [
  { timestamp: 100, sequence: 0, code: 1 },
  { timestamp: 100, sequence: 1, code: 2 },
  { timestamp: 160, sequence: 2, code: 0 }
];
const firstBoundary = Model.consumeInputTransitions(queued, 120, 0);
assert.equal(firstBoundary.code, 2, "same-timestamp transitions apply in sequence order");
