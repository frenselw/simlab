...
assert.doesNotMatch(html,/a11y-actions|data-reset-part|keyboard-help|鍵盤操作|方向鍵|Enter|Space/);for(const id of ["part1Controls","part2Controls","part3Controls"]){const section=html.match(new RegExp(`<section id="${id}"[\\s\\S]*?<\\/section>`))?.[0]||"";assert.doesNotMatch(section,/<button\\b/,`${id} has no normal-panel operational buttons`);}
...
assert.match(css,/\.support-target:hover[^}]*background:transparent!important/,"support hover stays transparent");assert.match(css,/\.support-target:focus-visible[^}]*outline:/,"support keeps a non-fill focus indication");
