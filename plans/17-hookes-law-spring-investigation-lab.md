...
- Learner task：設定兩條彈簧的原長 \(L_0\)（自然長度）及伸長量零位、完成共六次伸長量度、建立兩條通過原點的模型線、完成三次未量度負載的模型預測與一次最大安全負載設計，在提交前檢查答案後一次提交，再查看結果。
- Main interactions：彈簧 A／B button group、固定負載選擇、zero marker／measurement cursor／model handle／prediction marker 拖動及鍵盤替代、第三階段的彈簧＋負載聯動預測、跨階段返回／繼續、負載塊 `− / count / +`、review-edit、final submit。
...
- \(L_0\)：彈簧未加額外負載時的原長（自然長度），是伸長量的零位基準；
- \(F\)：所掛負載對彈簧施加的作用力；彈簧靜止平衡時，其大小等於彈簧回復力，單位 N；
- \(x\)：相對原長 \(L_0\) 的伸長量，\(x=L-L_0\)，單位 m；
- \(k\)：彈簧常數，單位 N/m；
...
所有學生可見的物理量、單位及模型都要使用一致的數學排版（HTML 使用 <var>／下標及不可拆分的數值－單位量，毋須載入 MathJax／KaTeX）：

- \(L_0\)：原長（自然長度），即未加額外負載時的彈簧長度；
- 伸長量 \(x=L-L_0\)：由原長零位起計的長度增加；
- 負載作用力 \(F\)：所掛負載對彈簧施加的力，單位 N；靜止平衡時與彈簧回復力大小相等；
- 彈簧常數／勁度 \(k\)：\(F-x\) 圖中學生模型直線的斜率，單位 N/m；\(k\) 越大，同一作用力造成的伸長越小；
- 學生模型：由自己的量度點建立的 \(F=kx\) 關係；保存或修改模型時顯示「模型」及「斜率」，不使用只有「模」字的標記；
...
只准正式記錄這三個負載。學生可任意次序量度及重做；第三階段的負載由另一個生成規則抽取，並排除這三個已量度的力值，以免直接試出第三階段的模型預測答案。
...
1. spring 保持無額外負載；
2. 固定直尺顯示絕對位置；
3. 「伸長量零位（原長基準）」水平 marker 由停泊位置開始；
...
### 8.3 Phase C：三個未量度負載的模型預測（重點重做）

第三階段必須先把學生要做的事說清楚，而不是只展示一條寫着「模」的線。每題的題目卡及舞台都要明確顯示：
...
- 記錄第三題後仍停留在 `predict` phase，只有學生明確按「完成三項預測後繼續」才進入第四階段。

舞台的初始狀態是有意設計的預測工作區，不是實際測試結果：

- 題目指定的彈簧及負載必須同時畫出來；
- 負載即使已畫在彈簧末端，也保持在未伸長位置，即原長基準 \(x=0\)；
- 舞台以天花板／固定端、彈簧、不同重量的不同大小／顏色負載及「未伸長位置（\(x=0\)）」基準線建立物理語境；
- 橙色 prediction marker 在彈簧及負載旁邊，學生拖動它時，彈簧、負載及預測位置必須同步伸長或縮短；
- 未伸長位置標籤與學生預測伸長標籤必須分開排版；prediction card 的數值與單位保持同一個不可拆分的數學量，手機窄屏也不可把數值／單位拆成不完整的多行；
...
- stage 頂部標題使用「預測 N・彈簧 A／B・負載下的伸長量」，直接說明學生要預測的物理量，不使用不通順的「F–x負載」；
- 舞台即時顯示學生自己的預測伸長量及由 \(L_0+x\) 得到的預測總長度，讓學生知道自己正在預測什麼；
...
### 8.4 Phase D：最大安全負載挑戰

題目示例：

> 彈簧的伸長不可超過 8.0 cm。請選擇彈簧及 0.5 N 負載塊數量，在安全範圍內令總作用力最大。

學生操作：

- 先選擇 spring A／B，選項旁顯示學生自己為該 spring 建立的 \(k\)；
- 用 `− / count / +` 每次調整一個 \(0.5\,\mathrm{N}\) 負載塊，並清楚顯示目前負載塊數量及總作用力；
- 舞台顯示清楚的「最大安全負載挑戰・找出安全方案」任務字句；縱軸 \(0\) 刻度與固定端／天花板處於同一水平位置，並畫出已選彈簧、紅色 \(x_{\max}\) 安全上限線及學生模型預測的末端位置；
- 未選擇彈簧時，提示文字依安全上限線的實際位置放在虛線下方，與安全上限標籤及刻度保持間隔，不可固定在可能重合的高度；
- 負載由彈簧下方垂直堆疊的細小灰色方塊表示；開始只有一粒，每增加一個負載塊就多一粒，整體負載向下延長；不另畫底部橫向負載塊圖示，也不把 \(\mathrm{N}\) 文字貼在方塊後方；
- 計算區用數學排版顯示：\(F=n\times0.5\,\mathrm{N}\)、\(x=F/k\) 及 \(x_{\max}\)，並用中性文字提醒學生自行比較；
- 顯示自己的方案摘要：彈簧、負載塊數量、總作用力；
...
- A／B 零位；
- 六項量度；
- 自己的兩條量度表（每條三個 \(F\)-\(x\) 數據點）；
- 自己的兩張 \(F-x\) 圖、模型線及模型斜率 \(k\)；
- 三項預測；
- 工程方案。
...
### 10.7 量度自動吸附（操作協助，不是評分容差）

- 第一階段的零位及穩定負載末端各有 0.003 m 的觸控／指標吸附範圍；學生把游標移到目標附近時，位置會吸附到該物理端點，讓手機操作可以取得準確讀數。
- 吸附只在互動位置轉換層執行，不顯示「接近」或正確性提示；保存後的 canonical meter coordinate 仍由 transition 驗證，評分仍使用本節既定的 0.002 m／0.0025 m 容差。
- 吸附邊界要以 inclusive threshold 加浮點誤差測試；同一負載的相同／epsilon 內位置是 no-op，不清除模型、預測或安全負載方案。
...
- 提交前檢查返回 edit 時 focus 到該 section heading，再由學生按 Tab 進入第一個控制；
