...
const undampedFastPull = P.stepDirectForce(directBreakaway, scenario.staticLimitMeanN + 5, scenario, .1);
const dampedSlowPull = P.stepDirectForce(directBreakaway, scenario.staticLimitMeanN + 5, scenario, .1, { dampingNsPerM: 36 });
assert.ok(dampedSlowPull.block.velocityMps < undampedFastPull.block.velocityMps, "the compact direct-pull response slows acceleration without changing the applied force");
assert.ok(dampedSlowPull.block.positionM < undampedFastPull.block.positionM, "the compact direct-pull response leaves more adjustment time on the stage");
